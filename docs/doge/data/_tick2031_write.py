# ephemeral tick2031 — WZC St Vincentius Antwerpen/Ekeren YE2025 Medium
import csv
import sys
...
SRC_KBO = "src_wzc_st_vincentius_antwerpen_kbo_2031"
SRC_SITE = "src_wzc_st_vincentius_antwerpen_site_2031"
...
for ns in [
    {
        **{k: "" for k in sfields},
        ...
        "notes": f"tick2031; YE2025 omzet JUMP {OMZET} pnl JUMP {PNL} equity JUMP {EQUITY} bruto JUMP {BRUTO} FTE {FTE}; neerlegging 17.07.2026; assets/debt Unknown; distinct from Avelgem; raw docs/doge/data/raw/tick2031/vincentius_ant_nl.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_EN,
        ...
        "accessed_date": "2026-08-24",
        "source_class": "secondary_aggregator",
        "notes": "tick2031; EN mirror YE2025 Medium; filed 17-07-2026; Last balance sheet year 2025; FTE 89.8; raw docs/doge/data/raw/tick2031/vincentius_ant_en.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_FR,
        ...
        "accessed_date": "2026-08-24",
        "source_class": "secondary_aggregator",
        "notes": "tick2031; FR mirror YE2025 Medium; déposés le 17-07-2026; raw docs/doge/data/raw/tick2031/vincentius_ant_fr.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_KBO,
        ...
        "notes": "tick2031; Actief VZW since 02.03.1978; Dorpstraat 32 2180 Antwerpen; 2 VE; KBO email inge.vriesacker@vincentiusekeren.be",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_SITE,
        ...
        "notes": "tick2031; info@vincentiusekeren.be",
    },
]:
...
for nb in [
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2031; omzet JUMP {OMZET} +1.54pct vs YE2024 {OMZET24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2031; pnl JUMP {PNL} +21.50pct vs YE2024 {PNL24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2031; equity JUMP {EQUITY} +1.19pct vs YE2024 {EQUITY24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2031; bruto JUMP {BRUTO} +1.87pct vs YE2024 {BRUTO24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2031; YE2025 FTE {FTE}",
    },
]:
...
nc = {
    **{k: "" for k in cfields},
    ...
    "title": "WZC St Vincentius Antwerpen/Ekeren YE2025 leftover dual (omzet JUMP 7.55m / pnl JUMP 0.43m / equity JUMP 12.45m)",
    "entity_id": ENTITY,
    ...
    "notes": "tick2031; Medium CW; assets/debt Unknown; DISTINCT from Avelgem; preferred AGB Bornem JR2024; FARO/AIESH/REW YE2024; Rillaar already mined EVERY-10",
}
...
nl = {
    **{k: "" for k in lfields},
    ...
    "status": "active",
    "struck_reason": "",
    "notes": "tick2031 leftover dual; Medium CW; distinct from Avelgem; TE-adjacent care flow not pure-waste top10; next every-10 2040",
}
...
ne = {
    **{k: "" for k in efields},
    ...
    "notes": "tick2031 YE2025 Medium CW NL+EN+FR + Strong KBO 0418.016.550 Actief VZW; omzet JUMP 7.55m pnl JUMP 0.43m equity JUMP 12.45m bruto JUMP 7.36m FTE 89.8; assets/debt Unknown; neerlegging 17.07.2026; 2 VE; DISTINCT from Avelgem; FOI "
    + GAP
    + "; preferred AGB Bornem JR2024; FARO/AIESH/REW YE2024; do not redo Sint-Jozef Rillaar/Karus/De Foyer/Ternat/Zilverbos/Mayerhof/Evara/Multiversum/Maria Rustoord/PPC Pittem/WZC Sint-Vincentius Avelgem/PC Sint-Hiëronymus/WZC Sint-Barbara/PC Gent-Sleidinge/AZ Rivierenland/AZ Zeno/HH Tienen/Heilig Hart Leuven/Sint-Trudo/Sint-Andries/Heilig Hart Lier/Vlaamse Zorgkas/OLVT/AZ Oostende",
}
...
- Distinct from WZC Sint-Vincentius Avelgem. Preferred stall: AGB Bornem JR2024; FARO/AIESH/REW YE2024. Sint-Jozef Rillaar already mined EVERY-10.
...
for x in qrows:
    if x.get("task_id") == "rq_2031":
        ...
        x["updated_utc"] = UTC
        x["entity_id"] = ENTITY
        x["title"] = "leftover dual hole-fill after WZC Sint-Jozef Rillaar — WZC St Vincentius Antwerpen/Ekeren YE2025 Medium"
        x["notes"] = (
            "tick2031 WZC St Vincentius Antwerpen Medium omzet JUMP 7.55m pnl JUMP 0.43m equity JUMP 12.45m; FOI ready; "
            "AGB Bornem JR2024; FARO/AIESH/REW YE2024; next rq_2032; next every-10 2040"
        )
        x["instructions"] = (
            "Completed leftover WZC St Vincentius Antwerpen/Ekeren YE2025 Medium CW; KBO 0418.016.550; "
    ...
    qrows.append(
        {
            **{k: "" for k in qfields},
            "task_id": "rq_2032",
            "title": "leftover dual hole-fill after WZC St Vincentius Antwerpen/Ekeren",
            "sprint": "hole_fill",
            "priority": "8",
            "status": "open",
            "hierarchy_target": "L5",
            "entity_id": "",
            "instructions": (
                "Tick 2031 after WZC St Vincentius Antwerpen/Ekeren YE2025 Medium. Prefer leftover AGB/APB if JR2025 PDF live, else FARO if TRUE NBB YE2025, "
                "else AIESH/REW if YE2025, else unused water/DSO/IGS/HVZ/energy/hospital/WZC/psych (OLV Lourdes Kortenberg YE2025 live deferred / OLVA Antwerpen YE2025 / Kanunnik Triest YE2025 / Cassiers / Sint-Bernardus Assenede / OLV Roosdaal / other unused YE2025 if live with omzet). "
            ...
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2031 WZC St Vincentius Antwerpen; next every-10 2040; Lourdes/OLVA/Triest YE2025 deferred",
        }
    )
...
lsrows[-1].update(
    {
        ...
        "last_tick_utc": UTC,
        "last_unit_id": "rq_2031",
        "ticks_completed": "2031",
        "paused": "no",
        "notes": (
            "tick2031 leftover WZC St Vincentius Antwerpen/Ekeren 0418.016.550 Medium CW (omzet JUMP 7.55m pnl JUMP 0.43m equity JUMP 12.45m bruto JUMP 7.36m FTE 89.8; "
            "assets/debt Unknown); AGB Bornem JR2024; FARO/AIESH/REW YE2024; next rq_2032; next every-10 2040; continuous hole_fill"
        ),
    }
)
...
## Tick 2031 - {UTC} - rq_2031 WZC St Vincentius Antwerpen/Ekeren (omzet JUMP 7.55m / pnl JUMP 0.43m / Medium)
...
- Wrote: sources (+5); budgets (+5); commitments (+1); leaderboard (+1); entities (+1 vzw_wzc_st_vincentius_antwerpen); foi + draft {GAP}; rq_2031=done + rq_2032 open; loop_state ticks=2031; raw under docs/doge/data/raw/tick2031/.
