# ephemeral fetch tick2035 OLV Roosdaal
import csv
import re
import shutil
import ssl
...
with open("docs/doge/data/entities.csv", encoding="utf-8", newline="") as f:
    for r in csv.DictReader(f):
        ...
        if "0421031171" in blob or (
            "roosdaal" in blob
            and ("olv" in blob or "onze-lieve" in blob or "wzc" in blob)
        ):
            print("ENT", r.get("entity_id"), (r.get("name_nl") or "")[:60])

outdir = Path("docs/doge/data/raw/tick2035")
outdir.mkdir(parents=True, exist_ok=True)
for src in [
    Path("docs/doge/data/raw/tick2034/roosdaal_en.html"),
    Path("docs/doge/data/raw/tick2031/olv_roosdaal_en.html"),
]:
    if src.exists():
        shutil.copy(src, outdir / "roosdaal_en.html")
        break
...
for name, url in [
    ("roosdaal_nl", "https://www.companyweb.be/nl/0421031171/w-z-c-onze-lieve-vrouw"),
    ("roosdaal_en", "https://www.companyweb.be/en/0421031171/w-z-c-onze-lieve-vrouw"),
    ("roosdaal_fr", "https://www.companyweb.be/fr/0421031171/w-z-c-onze-lieve-vrouw"),
    (
        "roosdaal_kbo",
        "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?ondernemingsnummer=0421031171",
    ),
    ("roosdaal_site", "https://www.wzc-olv-roosdaal.be/"),
