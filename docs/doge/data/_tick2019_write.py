# ephemeral tick2019 — WZC Sint-Barbara Herselt YE2025 Medium (leftover HVZ dual after PC Gent-Sleidinge)
import csv
import sys
...
SRC_KBO = "src_wzc_sint_barbara_herselt_kbo_2019"
SRC_SITE = "src_wzc_sint_barbara_herselt_site_2019"
...
r = next(x for x in qrows if x.get("task_id") == "rq_2019")
...
for ns in [
    {
        **{k: "" for k in sfields},
        ...
        "notes": f"tick2019; YE2025 omzet JUMP {OMZET} pnl DROP {PNL} equity JUMP {EQUITY} bruto JUMP {BRUTO} FTE {FTE}; neerlegging 19.06.2026; assets/debt Unknown; raw docs/doge/data/raw/tick2019/barbara_nl.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_EN,
        ...
        "accessed_date": "2026-08-24",
        "source_class": "secondary_aggregator",
        "notes": "tick2019; EN mirror YE2025 Medium; filed 19-06-2026; Last balance sheet year 2025; raw docs/doge/data/raw/tick2019/barbara_en.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_FR,
        ...
        "accessed_date": "2026-08-24",
        "source_class": "secondary_aggregator",
        "notes": "tick2019; FR mirror YE2025 Medium; deposés le 19-06-2026; raw docs/doge/data/raw/tick2019/barbara_fr.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_KBO,
        ...
        "notes": "tick2019; Actief VZW since 10.12.1981; Dieperstraat 17 2230 Herselt; no KBO email; 1 VE",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_SITE,
        ...
        "notes": "tick2019; info@sintbarbara.be",
    },
]:
...
for nb in [
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2019; omzet JUMP {OMZET} +1.76pct vs YE2024 {OMZET24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2019; pnl DROP {PNL} -8.58pct vs YE2024 {PNL24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2019; equity JUMP {EQUITY} +5.62pct vs YE2024 {EQUITY24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2019; bruto JUMP {BRUTO} +2.65pct vs YE2024 {BRUTO24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2019; YE2025 FTE {FTE}",
    },
]:
...
nc = {
    **{k: "" for k in cfields},
    ...
    "notes": "tick2019; Medium CW; assets/debt Unknown; preferred AGB Bornem JR2024; FARO/AIESH/REW YE2024; Jessa/ZOL CW N/A; PC Gent-Sleidinge already mined",
}
...
nl = {
    **{k: "" for k in lfields},
    ...
    "status": "active",
    "struck_reason": "",
    "notes": "tick2019 leftover HVZ dual; Medium CW; TE-adjacent care flow not pure-waste top10; next every-10 2020",
}
...
ne = {
    **{k: "" for k in efields},
    ...
    "notes": "tick2019 YE2025 Medium CW NL+EN+FR + Strong KBO 0422.152.314 Actief VZW; omzet JUMP 15.24m pnl DROP 1.76m equity JUMP 25.53m bruto JUMP 15.65m FTE 191.4; assets/debt Unknown; neerlegging 19.06.2026; 1 VE; FOI "
    + GAP
    + "; preferred AGB Bornem JR2024; FARO/AIESH/REW YE2024; Jessa/ZOL CW N/A; do not redo PC Gent-Sleidinge/AZ Rivierenland/AZ Zeno/HH Tienen/Heilig Hart Leuven/Sint-Trudo/Sint-Andries/Heilig Hart Lier/Vlaamse Zorgkas/OLVT/AZ Oostende/Werken Glorieux/AZ Alma/AZ St.-Elisabeth Herentals/Vitaz/Emmaüs/AZORG/Z.org KU Leuven/AZ Delta/AZJP/ZAS",
}
...
nf = {
    **{k: "" for k in ffields},
    ...
    "created_utc": UTC,
    "updated_utc": UTC,
    "notes": "tick2019; human-send only; Medium CW; next every-10 2020",
}
...
- Preferred stall: AGB Bornem JR2024; FARO/AIESH/REW YE2024. Jessa/ZOL CW N/A omzet. PC Gent-Sleidinge already mined.
...
for x in qrows:
    if x.get("task_id") == "rq_2019":
        x["status"] = "done"
        x["updated_utc"] = UTC
        x["entity_id"] = ENTITY
        x["title"] = "leftover dual hole-fill after PC Gent-Sleidinge — WZC Sint-Barbara Herselt YE2025 Medium"
        x["notes"] = (
            "tick2019 WZC Sint-Barbara Herselt Medium omzet JUMP 15.24m pnl DROP 1.76m equity JUMP 25.53m; FOI ready; "
            "AGB Bornem JR2024; FARO/AIESH/REW YE2024; Jessa/ZOL CW N/A; next rq_2020 EVERY-10; next every-10 2020"
        )
        x["instructions"] = (
...
if not any(x.get("task_id") == "rq_2020" for x in qrows):
    qrows.append(
        {
            **{k: "" for k in qfields},
            "task_id": "rq_2020",
            "title": "EVERY-10 + leftover dual hole-fill after WZC Sint-Barbara Herselt",
            "sprint": "hole_fill",
            "priority": "9",
            "status": "open",
            "hierarchy_target": "L5",
            "entity_id": "",
            "instructions": (
                "Tick 2019 after WZC Sint-Barbara Herselt YE2025 Medium. MANDATORY every-10: refresh progress_every_10_ticks.md + doge_waste_top10_current.md then hole-fill one unit. "
                "Prefer leftover AGB/APB if JR2025 PDF live, else FARO if TRUE NBB YE2025, else AIESH/REW if YE2025, "
                "else unused water/DSO/IGS/HVZ/energy/hospital/WZC (St Vincentius Antwerpen / Maria's Rustoord / Sint-Carolus / Zilverbos / other unused YE2025 if live with omzet). "
                "Do NOT redo WZC Sint-Barbara Herselt, PC Gent-Sleidinge, AZ Rivierenland, AZ Zeno, Heilig Hart Tienen, Heilig Hart Leuven, Sint-Trudo, Sint-Andries Tielt, Heilig Hart Lier, Vlaamse Zorgkas, OLVT/AZ Sint-Blasius, AZ Oostende, Damiaan shell, Werken Glorieux, AZ Alma, AZ St.-Elisabeth Herentals, Vitaz, Emmaüs, AZORG, Z.org KU Leuven, AZ Delta, AZJP, ZAS, CHR Verviers, CNDG, Haute Senne, CHBA, Saint-Luc, GHdC, Humani, CHIREC, CHU Tivoli, CHR Citadelle, ISoSL, Epicura, CHwapi, CHU UCL Namur, IDETA, SPI, Vivalia, IDELUX Finances, IFIGA, SOFILUX, IDEFIN, FINIMO, FINEST, HYGEA, "
            ...
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2019 WZC Sint-Barbara Herselt; EVERY-10 mandatory at 2020; Jessa/ZOL CW N/A",
        }
    )
...
lsrows[-1].update(
    {
        ...
        "last_tick_utc": UTC,
        "last_unit_id": "rq_2019",
        "ticks_completed": "2019",
        "paused": "no",
        "notes": (
            "tick2019 leftover WZC Sint-Barbara Herselt 0422.152.314 Medium CW (omzet JUMP 15.24m pnl DROP 1.76m equity JUMP 25.53m bruto JUMP 15.65m FTE 191.4; "
            "assets/debt Unknown); AGB Bornem JR2024; FARO/AIESH/REW YE2024; Jessa/ZOL CW N/A; next rq_2020 EVERY-10; continuous hole_fill"
        ),
    }
)
...
## Tick 2019 - {UTC} - rq_2019 WZC Sint-Barbara Herselt (omzet JUMP 15.24m / pnl DROP 1.76m / Medium)
...
- Wrote: sources (+5); budgets (+5); commitments (+1); leaderboard (+1); entities (+1 vzw_wzc_sint_barbara_herselt); foi + draft {GAP}; rq_2019=done + rq_2020 open (EVERY-10); loop_state ticks=2019; raw under docs/doge/data/raw/tick2019/.
- FOI: **ready not sent** (human-gated; info@sintbarbara.be).
- NOT every-10 (**next every-10 is 2020**). Next: rq_2020 (EVERY-10 + AGB/FARO-if-YE2025 / AIESH-REW / St Vincentius-Maria Rustoord / unused DSO-IGS-HVZ).
"""
