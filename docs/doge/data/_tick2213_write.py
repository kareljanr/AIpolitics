# -*- coding: utf-8 -*-
"""Tick 2213 leftover dual — OptimaT / Huize Tordale YE2025 Medium."""
import csv
import json
...
append_csv(
    ...
    [
        dict(
            ...
            notes=f"tick2213; YE2025 omzet JUMP {OMZET} (+2.41%) bruto JUMP {BRUTO} (≫omzet ~{RATIO}x) pnl JUMP {PNL} (+13.7%) equity JUMP {EQUITY} (+3.75%) FTE JUMP {FTE}; neerlegging 09.06.2026; raw docs/doge/data/raw/tick2213/",
        ),
        dict(
            source_id=SRC_EN,
            ...
            accessed_date="2026-08-26",
            source_class="secondary_aggregator",
            notes=f"tick2213; EN mirror YE2025 Medium; filed 09-06-2026; Turnover {OMZET}; Gross margin {BRUTO}; Profit/Loss {PNL}; Equity {EQUITY}; Employees {FTE}; raw tick2213/",
        ),
        dict(
            ...
            accessed_date="2026-08-26",
            source_class="secondary_aggregator",
            notes=f"tick2213; FR mirror YE2025 Medium; Dernier bilan 2025; CA {OMZET}; Marge brute {BRUTO}; Benefice {PNL}; Capitaux propres {EQUITY}; raw tick2213/",
        ),
        dict(
            source_id="src_optimat_kbo_2213",
            ...
            notes="tick2213; Actief VZW sinds 02.04.1984; naam Huize Tordale; zetel Bruggestraat 39 8820 Torhout; 4 VE; RSZ/BTW NACE 88.993; trading OptimaT Lichtervelde",
        ),
        dict(
            source_id="src_optimat_site_contact_2213",
            title="OptimaT FOI channel info@optimat.be",
            url="https://optimat.be/",
            publisher="OptimaT VZW / Huize Tordale",
            accessed_date="2026-08-26",
            source_class="foi_contact",
            notes="tick2213; info@optimat.be; +32 51 68 02 00; Kortemarkstraat 86 8810 Lichtervelde (ops); KBO zetel Bruggestraat 39 Torhout",
        ),
    ],
)
...
for bid, year, amount, basis, notes in [
    ("bud_optimat_omzet_jr2025_statutory", "2025", OMZET, "CW statutory omzet / Turnover YE2025", f"tick2213; Medium CW; omzet JUMP +2.41% vs YE2024 {OMZET_PY}; primary envelope"),
    ("bud_optimat_bruto_jr2025_statutory", "2025", BRUTO, "CW statutory bruto_marge / Gross margin YE2025", f"tick2213; Medium CW; bruto JUMP +6.75% vs YE2024 {BRUTO_PY}; bruto≫omzet ~{RATIO}x"),
    ("bud_optimat_pnl_jr2025_statutory", "2025", PNL, "CW statutory winst / Profit-Loss after tax YE2025", f"tick2213; Medium CW; pnl JUMP +13.7% vs YE2024 {PNL_PY}"),
    ("bud_optimat_equity_jr2025_statutory", "2025", EQUITY, "CW statutory eigen_vermogen / Equity YE2025", f"tick2213; Medium CW; equity JUMP +3.75% vs YE2024 {EQUITY_PY}"),
    ("bud_optimat_fte_jr2025_statutory", "2025", FTE, f"CW social-balance FTE / Employees {FTE}", f"tick2213; Medium CW; FTE JUMP vs YE2024 {FTE_PY}; assets/debt Unknown"),
    ("bud_optimat_omzet_jr2024_statutory_cmp", "2024", OMZET_PY, "CW statutory omzet YE2024 comparative", f"tick2213; YE2024 omzet {OMZET_PY} comparative"),
]:
    budgets.append(
        dict(
            budget_id=bid,
...
append_csv(
    ...
    [
        dict(
            ...
            notes=f"tick2213; Medium CW; omzet primary; bruto≫omzet (~{RATIO}x) extreme + equity JUMP 39.4m + FTE 788.8; assets/debt Unknown; preferred AGB Bornem JR2024; FARO/REW YE2024; not TE-additive of 348bn; do not redo Ecoso/Werkhuizen Min/ACG/Noordheuvel/Arcor/Kemphaan/Entiris/Oesterbank",
        )
    ],
)
...
append_csv(
    ...
    [
        dict(
            ...
            status="open",
            struck_reason="",
            notes=f"tick2213; Medium CW; FOI {GAP}; stall FARO/REW YE2024; AGB Bornem JR2024; West-Vlaanderen maatwerk dual after Ecoso",
        )
    ],
)
...
append_csv(
    ...
    [
        dict(
            ...
            notes=f"tick2213 YE2025 Medium CW NL+EN+FR + Strong KBO 0429.649.325 Actief VZW Huize Tordale 4 VE RSZ NACE 88.993; omzet JUMP {OMZET} bruto {BRUTO} (≫omzet ~{RATIO}x) pnl JUMP {PNL} equity JUMP {EQUITY} FTE JUMP {FTE}; neerlegging 09.06.2026; KBO zetel Bruggestraat 39 Torhout; assets/debt Unknown; FOI {GAP}; preferred AGB Bornem JR2024; FARO/REW YE2024; not TE-additive of 348bn",
        )
    ],
)
...
append_csv(
    ...
    [
        dict(
            ...
            created_utc=TS,
            updated_utc=TS,
            notes="tick2213; ready NOT sent; Medium CW + Strong KBO; stall FARO/REW YE2024; AGB Bornem JR2024; next every-10 2220",
        )
    ],
)
...
update_csv_rows(
    ...
    "task_id",
    {
        "rq_2213": {
            ...
            "instructions": "Completed leftover OptimaT after Ecoso; preferred AGB Bornem JR2024 / FARO/AIESH/REW still YE2024; Medium CW YE2025 + Strong KBO; FOI ready not sent",
            "notes": f"tick2213 OptimaT/Huize Tordale 0429.649.325 Medium; omzet JUMP {OMZET} bruto {BRUTO} (~{RATIO}x) pnl JUMP {PNL} equity JUMP {EQUITY} FTE JUMP {FTE}; 4 VE; AGB Bornem JR2024; FARO/REW YE2024; next rq_2214; next every-10 2220",
        }
    },
)
    ...
    [
        dict(
            ...
            instructions=(
                ...
                "Do NOT redo OptimaT, Huize Tordale, Ecoso, Werkhuizen Min, ACG, Noordheuvel, Arcor, Kemphaan, Entiris, Oesterbank, Werkplus, Trianval, "
            ...
            created_utc=TS,
            updated_utc=TS,
            notes="spawned after tick2213 OptimaT; FARO/AIESH/REW still YE2024; AGB Bornem JR2024; next EVERY-10 at 2220",
        )
    ],
)
...
for row in rows:
    if row.get("state_id") == "main":
        ...
        row["paused"] = "no"
        row["notes"] = (
            f"tick2213 leftover OptimaT/Huize Tordale 0429.649.325 Medium (omzet JUMP 11.92m; bruto 42.26m ≫ omzet ~{RATIO}x; "
            f"pnl JUMP 1.91m; equity JUMP 39.41m; FTE JUMP 788.8; 4 VE); "
            "AGB Bornem JR2024; FARO/REW YE2024; next rq_2214; next every-10 2220; continuous hole_fill"
        )
...
log_block = f"""
## Tick 2213 - {TS} - rq_2213 OptimaT / Huize Tordale (omzet JUMP 11.92m / bruto≫omzet ~{RATIO}x / equity JUMP 39.4m / Medium)

- Unit: **rq_2213** leftover dual after **rq_2212 Ecoso**. Prefer NON-stall live: AGB Bornem still **JR2024-only**; FARO still **YE2024**; REW still **YE2024**. Took named FREE leftover **Huize Tordale VZW trading OptimaT** YE2025 (KBO **0429.649.325**; Bruggestraat 39 Torhout / Kortemarkstraat 86 Lichtervelde; **Actief** **4 VE**; RSZ NACE **88.993**). Do not redo Ecoso/Werkhuizen Min/ACG/Noordheuvel/Arcor/Kemphaan/Entiris/Oesterbank.
...
- Wrote: sources (+5); budgets (+6); commitments (+1); leaderboard (+1 pi {PI}); entities (+1 {ENTITY}); foi + draft {GAP}; rq_2213=done + rq_2214 open; loop_state ticks=2213; raw docs/doge/data/raw/tick2213/.
- FOI: **ready not sent** (human-gated).
- NOT every-10 (**last every-10 was 2210**; next **2220**). Next: rq_2214 (AGB/FARO-if-YE2025 / AIESH-REW / Manus-Constructief-or-unused).

"""
with LOG.open("a", encoding="utf-8") as f:
    f.write(log_block)
print("loop_log appended")
print("DONE tick2213 OptimaT")
