# ephemeral tick2033 — Cassiers WZC Houthulst YE2025 Medium
import csv
import sys
...
r = next(x for x in qrows if x.get("task_id") == "rq_2033")
...
for ns in [
    {
        **{k: "" for k in sfields},
        ...
        "notes": f"tick2033; YE2025 omzet JUMP {OMZET} pnl DROP {PNL} equity JUMP {EQUITY} bruto FLAT {BRUTO} FTE {FTE}; neerlegging 03.07.2026; assets/debt Unknown; raw docs/doge/data/raw/tick2033/cassiers_nl.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_EN,
        ...
        "accessed_date": "2026-08-24",
        "source_class": "secondary_aggregator",
        "notes": "tick2033; EN mirror YE2025 Medium; filed 03-07-2026; Last balance sheet year 2025; FTE 97.7; raw docs/doge/data/raw/tick2033/cassiers_en.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_FR,
        ...
        "accessed_date": "2026-08-24",
        "source_class": "secondary_aggregator",
        "notes": "tick2033; FR mirror YE2025 Medium; déposés le 03-07-2026; raw docs/doge/data/raw/tick2033/cassiers_fr.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_KBO,
        ...
        "notes": "tick2033; Actief VZW since 09.12.1987; 7e-Geniestraat 3 8650 Houthulst; 3 VE; raw docs/doge/data/raw/tick2033/cassiers_kbo.html",
    },
    {
        **{k: "" for k in sfields},
        "source_id": SRC_SITE,
        ...
        "notes": "tick2033; info@wzccassiers.be; +32 51 70 81 90; 7e Geniestraat 3 8650 Houthulst",
    },
]:
...
for nb in [
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2033; omzet JUMP {OMZET} +1.90pct vs YE2024 {OMZET24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2033; pnl DROP {PNL} -27.69pct vs YE2024 {PNL24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2033; equity JUMP {EQUITY} +10.11pct vs YE2024 {EQUITY24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": f"tick2033; bruto FLAT {BRUTO} -0.06pct vs YE2024 {BRUTO24}",
    },
    {
        ...
        "source_id": SRC,
        "confidence": "medium",
        "notes": "tick2033; YE2025 FTE 97.7 vs YE2024 98.2",
    },
]:
...
nc = {
    **{k: "" for k in cfields},
    ...
    "notes": "tick2033; Medium CW; assets/debt Unknown; preferred AGB Bornem JR2024; FARO/AIESH/REW YE2024; Bernardus Assenede/OLV Roosdaal/Lourdes/OLVA/Triest deferred",
}
...
nl = {
    **{k: "" for k in lfields},
    ...
    "status": "active",
    "struck_reason": "",
    "notes": "tick2033 leftover WZC dual; Medium CW; TE-adjacent care flow not pure-waste top10; next every-10 2040",
}
...
ne = {
    **{k: "" for k in efields},
    ...
    "notes": "tick2033 YE2025 Medium CW NL+EN+FR + Strong KBO 0434.434.393 Actief VZW; omzet JUMP 7.20m pnl DROP 0.42m (-27.69pct) equity JUMP 9.74m (+10.11pct) bruto FLAT 7.76m FTE 97.7; assets/debt Unknown; neerlegging 03.07.2026; 3 VE; FOI "
    + GAP
    + "; preferred AGB Bornem JR2024; FARO/AIESH/REW YE2024; do not redo Vincentius Antwerpen/Rillaar/Karus/De Foyer/Ternat/Zilverbos/Mayerhof/Evara/Maria Rustoord/PPC Pittem/Vincentius Avelgem/PC Sint-Hiëronymus/WZC Sint-Barbara",
}
...
nf = {
    **{k: "" for k in ffields},
    ...
    "created_utc": UTC,
    "updated_utc": UTC,
    "notes": "tick2033; human-send only; Medium CW; next every-10 2040",
}
...
- Preferred stall: AGB Bornem JR2024; FARO/AIESH/REW YE2024. Bernardus Assenede / OLV Roosdaal / OLVA / Triest YE2025 deferred.
...
skip_list = (
    "Do NOT redo Cassiers WZC, WZC OLV Lourdes Kortenberg, WZC St Vincentius Antwerpen/Ekeren, WZC Sint-Jozef Rillaar, Karus, WZC De Foyer Gent, "
    "WZC Sint-Carolus Ternat, WZC Zilverbos Zelzate, Sint Carolus Mayerhof, Evara/Multiversum, Maria Rustoord Ingelmunster, "
    "PPC Pittem, WZC Sint-Vincentius Avelgem, PC Sint-Hiëronymus, WZC Sint-Barbara Herselt, PC Gent-Sleidinge, Veilige Have, "
...
for x in qrows:
    if x.get("task_id") == "rq_2033":
        ...
        x["updated_utc"] = UTC
        x["entity_id"] = ENTITY
        x["title"] = "leftover dual hole-fill after WZC OLV Lourdes Kortenberg — Cassiers WZC YE2025 Medium"
        x["notes"] = (
            "tick2033 Cassiers Medium omzet JUMP 7.20m pnl DROP 0.42m (-27.69pct) equity JUMP 9.74m; FOI ready; "
            "AGB Bornem JR2024; FARO/AIESH/REW YE2024; next rq_2034; next every-10 2040"
        )
        x["instructions"] = (
...
if not any(x.get("task_id") == "rq_2034" for x in qrows):
    qrows.append(
        {
            **{k: "" for k in qfields},
            "task_id": "rq_2034",
            ...
            "entity_id": "",
            "instructions": (
                "Tick 2033 after Cassiers WZC YE2025 Medium. Prefer leftover AGB/APB if JR2025 PDF live, else FARO if TRUE NBB YE2025, "
                "else AIESH/REW if YE2025, else unused water/DSO/IGS/HVZ/energy/hospital/WZC/psych "
                "(Sint-Bernardus Assenede YE2025 live deferred / OLV Roosdaal YE2025 / OLVA Antwerpen YE2025 / "
                "Kanunnik Triest YE2025 / other unused YE2025 if live with omzet). "
                + skip_list
            ),
            "blocked_gap_id": "",
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2033 Cassiers; next every-10 2040",
        }
    )
save("docs/doge/data/research_queue.csv", qrows, qfields)
print("queue updated")
...
for x in srows2:
    if x.get("state_id") == "main":
        ...
        x["last_tick_utc"] = UTC
        x["last_unit_id"] = "rq_2033"
        x["ticks_completed"] = "2033"
        x["paused"] = "no"
        x["notes"] = (
            "tick2033 leftover Cassiers WZC 0434.434.393 Medium CW (omzet JUMP 7.20m pnl DROP 0.42m -27.69pct "
            "equity JUMP 9.74m bruto FLAT 7.76m FTE 97.7; assets/debt Unknown); AGB Bornem JR2024; FARO/AIESH/REW YE2024; "
            "next rq_2034; next every-10 2040; continuous hole_fill"
        )
...
## Tick 2033 - {UTC} - rq_2033 Cassiers WZC (omzet JUMP 7.20m / pnl DROP 0.42m / Medium)
