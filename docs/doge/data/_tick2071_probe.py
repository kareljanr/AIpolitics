# ephemeral probe tick2071 — claim + stalls + candidates
import csv
import re
import ssl
import sys
...
cands = [
    ...
    # unused WZC / zorg candidates (avoid redo list)
    ("huize_zusters", "https://www.companyweb.be/en/0405548739"),
    ("wzc_zonnehoeve", "https://www.companyweb.be/en/0426213886"),
    ("wzc_immaculata", "https://www.companyweb.be/en/0411998612"),
    ("wzc_bethanie", "https://www.companyweb.be/en/0412115144"),
    ("wzc_de_mijlpaal", "https://www.companyweb.be/en/0453892285"),
]
...
for name, url in cands:
    try:
        ...
        print(
            ...
            (title.group(1)[:65] if title else ""),
            "blocks",
            parse_blocks(html)[:2],
        )
...
# entity collision check for msw
...
for n, k in [
    ...
    hits = [
        e.get("entity_id")
        for e in ents
        if n.lower() in ((e.get("entity_id") or "") + (e.get("name_nl") or "") + (e.get("notes") or "")).lower()
