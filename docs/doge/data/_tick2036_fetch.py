# ephemeral fetch tick2036 OLVA Antwerpen
import re
import shutil
import ssl
...
outdir = Path("docs/doge/data/raw/tick2036")
outdir.mkdir(parents=True, exist_ok=True)
for src in [
    Path("docs/doge/data/raw/tick2034/olva_en.html"),
    Path("docs/doge/data/raw/tick2030/olva_en.html"),
]:
    if src.exists():
        shutil.copy(src, outdir / "olva_en.html")
        break
...
for name, url in [
    (
        "olva_nl",
        "https://www.companyweb.be/nl/0430977136/woon-en-zorgcentrum-onze-lieve-vrouw-van-antwerpen",
    ),
    (
        "olva_en",
    ...
    (
        "olva_fr",
        "https://www.companyweb.be/fr/0430977136/woon-en-zorgcentrum-onze-lieve-vrouw-van-antwerpen",
    ),
    (
        "olva_kbo",
        "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?ondernemingsnummer=0430977136",
    ),
    ("olva_site", "https://www.olva.be/"),
    ("olva_site2", "https://www.wzc-olva.be/"),
]:
    try:
        ...
        if "kbo" in name or "site" in name:
            ...
            if "kbo" in name:
                ...
                if m:
                    print(" addr", re.sub(r"<[^>]+>", " ", m.group(0))[:180])
            continue
        year = None
