# ephemeral fetch tick2037 Kanunnik Triest
import csv
import re
import shutil
import ssl
...
with open("docs/doge/data/entities.csv", encoding="utf-8", newline="") as f:
    for r in csv.DictReader(f):
        ...
        if "0410509443" in blob or "triest" in blob and "kanunnik" in blob:
            print("ENT", r.get("entity_id"), (r.get("name_nl") or "")[:60])

outdir = Path("docs/doge/data/raw/tick2037")
outdir.mkdir(parents=True, exist_ok=True)
for src in [
    Path("docs/doge/data/raw/tick2034/triest_en.html"),
    Path("docs/doge/data/raw/tick2030/triest_en.html"),
]:
    if src.exists():
        shutil.copy(src, outdir / "triest_en.html")
        break
...
for name, url in [
    (
        "triest_nl",
        "https://www.companyweb.be/nl/0410509443/woonzorgcentrum-kanunnik-triest-vzw",
    ),
    (
        "triest_en",
    ...
    (
        "triest_fr",
        "https://www.companyweb.be/fr/0410509443/woonzorgcentrum-kanunnik-triest-vzw",
    ),
    (
        "triest_kbo",
        "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?ondernemingsnummer=0410509443",
    ),
    ("triest_site", "https://www.wzckanunniktriest.be/"),
    ("triest_site2", "https://www.kanunniktriest.be/"),
