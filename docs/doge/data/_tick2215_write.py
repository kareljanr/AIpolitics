# -*- coding: utf-8 -*-
"""Tick 2215 leftover dual — Constructief Kortrijk YE2025 Medium."""
import csv
import json
...
append_csv(
    ...
    [
        dict(
            ...
            notes=f"tick2215; YE2025 omzet JUMP {OMZET} (+6.06%) bruto JUMP {BRUTO} (~{RATIO}x) pnl JUMP {PNL} (+292.96%) equity DROP {EQUITY} (-2.44%) FTE JUMP {FTE}; neerlegging 09.06.2026; raw docs/doge/data/raw/tick2215/",
        ),
        dict(
            source_id=SRC_EN,
            ...
            accessed_date="2026-08-26",
            source_class="secondary_aggregator",
            notes=f"tick2215; EN mirror YE2025 Medium; filed 09-06-2026; Turnover {OMZET}; Gross margin {BRUTO}; Profit/Loss {PNL}; Equity {EQUITY}; Employees {FTE}; raw tick2215/",
        ),
        dict(
            ...
            accessed_date="2026-08-26",
            source_class="secondary_aggregator",
            notes=f"tick2215; FR mirror YE2025 Medium; Dernier bilan 2025; CA {OMZET}; Marge brute {BRUTO}; Benefice {PNL}; Capitaux propres {EQUITY}; raw tick2215/",
        ),
        dict(
            source_id="src_constructief_kbo_2215",
            ...
            notes="tick2215; Actief VZW sinds 18.12.1998; zetel Warande(Heu) 7 8501 Kortrijk; 1 VE; RSZ/BTW NACE 88.993; Deltagroep sister of Mobiel/Kringloop ZWVL",
        ),
        dict(
            source_id="src_constructief_site_contact_2215",
            title="Constructief FOI channel info@vzwconstructief.be",
            url="https://vzwconstructief.be/",
            publisher="Constructief VZW",
            accessed_date="2026-08-26",
            source_class="foi_contact",
            notes="tick2215; info@vzwconstructief.be; +32 56 36 28 02; Warande 7 8501 Kortrijk; hout/groen maatwerk",
        ),
    ],
)
...
for bid, year, amount, basis, notes in [
    ("bud_constructief_omzet_jr2025_statutory", "2025", OMZET, "CW statutory omzet / Turnover YE2025", f"tick2215; Medium CW; omzet JUMP +6.06% vs YE2024 {OMZET_PY}; primary envelope"),
    ("bud_constructief_bruto_jr2025_statutory", "2025", BRUTO, "CW statutory bruto_marge / Gross margin YE2025", f"tick2215; Medium CW; bruto JUMP +9.01% vs YE2024 {BRUTO_PY}; bruto/omzet ~{RATIO}x"),
    ("bud_constructief_pnl_jr2025_statutory", "2025", PNL, "CW statutory winst / Profit-Loss after tax YE2025", f"tick2215; Medium CW; pnl JUMP +292.96% vs YE2024 {PNL_PY}"),
    ("bud_constructief_equity_jr2025_statutory", "2025", EQUITY, "CW statutory eigen_vermogen / Equity YE2025", f"tick2215; Medium CW; equity DROP -2.44% vs YE2024 {EQUITY_PY}"),
    ("bud_constructief_fte_jr2025_statutory", "2025", FTE, f"CW social-balance FTE / Employees {FTE}", f"tick2215; Medium CW; FTE JUMP vs YE2024 {FTE_PY}; assets/debt Unknown"),
    ("bud_constructief_omzet_jr2024_statutory_cmp", "2024", OMZET_PY, "CW statutory omzet YE2024 comparative", f"tick2215; YE2024 omzet {OMZET_PY} comparative"),
]:
    budgets.append(
        dict(
            budget_id=bid,
...
append_csv(
    ...
    [
        dict(
            ...
            notes=f"tick2215; Medium CW; omzet primary; pnl JUMP +293% with equity DROP -2.44% + FTE JUMP; Deltagroep sister; assets/debt Unknown; preferred AGB Bornem JR2024; FARO/REW YE2024; not TE-additive of 348bn; do not redo OptimaT/Odas/Ecoso/Werkhuizen Min/ACG",
        )
    ],
)
...
append_csv(
    ...
    [
        dict(
            ...
            status="open",
            struck_reason="",
            notes=f"tick2215; Medium CW; FOI {GAP}; stall FARO/REW YE2024; AGB Bornem JR2024; after OptimaT",
        )
    ],
)
...
append_csv(
    ...
    [
        dict(
            ...
            notes=f"tick2215 YE2025 Medium CW NL+EN+FR + Strong KBO 0465.225.262 Actief VZW 1 VE RSZ NACE 88.993; omzet JUMP {OMZET} bruto {BRUTO} (~{RATIO}x) pnl JUMP {PNL} (+293%) equity DROP {EQUITY} FTE JUMP {FTE}; neerlegging 09.06.2026; Deltagroep with Mobiel/Kringloop ZWVL; assets/debt Unknown; FOI {GAP}; preferred AGB Bornem JR2024; FARO/REW YE2024; not TE-additive of 348bn",
        )
    ],
)
...
append_csv(
    ...
    [
        dict(
            ...
            created_utc=TS,
            updated_utc=TS,
            notes="tick2215; ready NOT sent; Medium CW + Strong KBO; stall FARO/REW YE2024; AGB Bornem JR2024; next every-10 2220",
        )
    ],
)
...
update_csv_rows(
    ...
    "task_id",
    {
        "rq_2215": {
            ...
            "instructions": "Completed leftover Constructief after OptimaT; preferred AGB Bornem JR2024 / FARO/AIESH/REW still YE2024; Medium CW YE2025 + Strong KBO; FOI ready not sent",
            "notes": f"tick2215 Constructief 0465.225.262 Medium; omzet JUMP {OMZET} bruto {BRUTO} (~{RATIO}x) pnl JUMP {PNL} (+293%) equity DROP {EQUITY} FTE JUMP {FTE}; 1 VE Kortrijk Deltagroep; AGB Bornem JR2024; FARO/REW YE2024; next rq_2216; next every-10 2220",
        }
    },
)
    ...
    [
        dict(
            ...
            instructions=(
                ...
                "Do NOT redo Constructief, OptimaT, Huize Tordale, Odas, Ecoso, Werkhuizen Min, ACG, Noordheuvel, Arcor, Kemphaan, Entiris, Oesterbank, "
            ...
            created_utc=TS,
            updated_utc=TS,
            notes="spawned after tick2215 Constructief; FARO/AIESH/REW still YE2024; AGB Bornem JR2024; next EVERY-10 at 2220",
        )
    ],
)
...
for row in rows:
    if row.get("state_id") == "main":
        ...
        row["paused"] = "no"
        row["notes"] = (
            f"tick2215 leftover Constructief 0465.225.262 Medium (omzet JUMP 2.11m; bruto 3.21m ~{RATIO}x; "
            f"pnl JUMP 21k +293%; equity DROP 956k; FTE JUMP 86.8; 1 VE Kortrijk Deltagroep); "
            "AGB Bornem JR2024; FARO/REW YE2024; next rq_2216; next every-10 2220; continuous hole_fill"
        )
...
log_block = f"""
## Tick 2215 - {TS} - rq_2215 Constructief Kortrijk (omzet JUMP 2.11m / pnl JUMP +293% / bruto≫omzet ~{RATIO}x / Medium)

- Unit: **rq_2215** leftover dual after **rq_2214 OptimaT**. Prefer NON-stall live: AGB Bornem still **JR2024-only**; FARO still **YE2024**; REW still **YE2024**. Took named FREE leftover **Constructief VZW** YE2025 (KBO **0465.225.262**; Warande 7 Kortrijk; **Actief** **1 VE**; RSZ NACE **88.993**; Deltagroep). Do not redo OptimaT/Odas/Ecoso/Werkhuizen Min/ACG/Noordheuvel/Arcor/Kemphaan/Entiris/Oesterbank.
...
- Wrote: sources (+5); budgets (+6); commitments (+1); leaderboard (+1 pi {PI}); entities (+1 {ENTITY}); foi + draft {GAP}; rq_2215=done + rq_2216 open; loop_state ticks=2215; raw docs/doge/data/raw/tick2215/.
- FOI: **ready not sent** (human-gated).
- NOT every-10 (**last every-10 was 2210**; next **2220**). Next: rq_2216 (AGB/FARO-if-YE2025 / AIESH-REW / Mobiel-Opnieuw-or-unused).

"""
with LOG.open("a", encoding="utf-8") as f:
    f.write(log_block)
print("loop_log appended")
print("DONE tick2215 Constructief")
