# ephemeral tick2010 — claim + FARO/AIESH/REW + Vlaamse Zorgkas YE2025 EVERY-10
import csv
import re
import ssl
import sys
...
r = next(x for x in rows if x.get("task_id") == "rq_2010")
...
with p.open("w", encoding="utf-8", newline="") as f:
    ...
    w.writeheader()
    w.writerows(rows)
print("claimed rq_2010")

dst = Path("docs/doge/data/raw/tick2010")
dst.mkdir(parents=True, exist_ok=True)
# reuse zorgkas from tick2009 if present
src9 = Path("docs/doge/data/raw/tick2009")
for name in ["zorgkas_nl", "zorgkas_en"]:
    s = src9 / f"{name}.html"
    if s.exists() and not (dst / f"{name}.html").exists():
        (dst / f"{name}.html").write_bytes(s.read_bytes())
        print("COPIED", name)
...
urls = [
    ...
    ("zorgkas_fr", "https://www.companyweb.be/fr/0475581694/vlaamse-zorgkas"),
    (
        "zorgkas_kbo",
        "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?ondernemingsnummer=0475581694",
    ),
    ("zorgkas_site", "https://www.vlaamsezorgkas.be/"),
]
