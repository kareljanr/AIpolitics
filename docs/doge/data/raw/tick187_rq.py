from pathlib import Path
import re
p = Path("docs/doge/data/research_queue.csv")
raw = p.read_bytes()
for enc in ["utf-8", "utf-8-sig", "cp1252", "latin-1"]:
    try:
        t = raw.decode(enc)
        break
    except Exception:
        pass
else:
    raise SystemExit("decode fail")
m = re.search(r"^rq_182,.*$", t, re.M)
if not m:
    raise SystemExit("rq_182 missing")
new = (
    'rq_182,Continuous FOI-adjacent public hole-fill batch,continuous,5,done,L5,gg_belgium,'
    '"Prefer public primary fills (Port Antwerp-Bruges authority accounts Charleroi BSCA dual Credendo North Sea Port Antwerp register Mons BI2026 FPS taxex) if new PDFs appear; else next open rq; do not idle while public work remains.",'
    'gap_sowaer_accounts,2026-07-28T12:05:00Z,2026-07-28T12:25:00Z,'
    '"tick187: BSCA AR2024 omzet 126.9m EBITDA 28.6m net 21.4m pax 10.5m dual BAC; SOWAER 35.9pct FOI ready; Port/Credendo residual; spawn rq_183"'
)
seed = (
    'rq_183,Continuous FOI-adjacent public hole-fill batch,continuous,5,open,L5,gg_belgium,'
    '"Prefer public primary fills (Credendo export credit Port Antwerp-Bruges authority North Sea Port Antwerp register Mons BI2026 FPS taxex) if new PDFs appear; else next open rq; do not idle while public work remains.",'
    ',2026-07-28T12:25:00Z,,'
    '"Spawned tick187 after BSCA dual; rq_116 SWA deferred Oct-Dec 2026"'
)
t2 = t[: m.start()] + new + t[m.end() :]
if not t2.endswith("\n"):
    t2 += "\n"
t2 += seed + "\n"
p.write_bytes(t2.encode(enc))
print("rq ok")
