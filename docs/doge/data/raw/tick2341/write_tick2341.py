...
rq = next((row for row in rq_rows if row["task_id"] == "rq_2341"), None)
if not rq or rq.get("status") not in ("open", "in_progress"):
    raise SystemExit("rq_2341 not claimable: " + repr(rq and rq.get("status")))
if has_id(ROOT / "entities.csv", "entity_id", ENTITY):
    raise SystemExit("De Korenbloem already present")
for row in rq_rows:
    if row["task_id"] == "rq_2341":
...
if not has_id(ROOT / "sources.csv", "source_id", "src_korenbloem_jr2025_cw_en"):
    for sid, title, url, pub, klass, notes in [
        (
            ...
            "tick2341; YE2025 omzet JUMP 12564145 bruto JUMP 12311379 pnl LOSS FLIP -360537 equity DROP 8688763 FTE JUMP 175.9; filed 03-06-2026",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            "tick2341; EN Medium; Turnover 12564145 Gross 12311379 P/L -360537 Equity 8688763 FTE 175.9; filed 03-06-2026",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            "tick2341; FR mirror YE2025 CA 12564145 marge 12311379 perte -360537 equity 8688763 FTE 175.9",
        ),
        (
            ...
            "KBO FOD Economie",
            "official_register",
            "tick2341; Actief VZW 1 VE Kortrijk Aanbestedende; RSZ 87.101 RVT; zetel Pieter de Conincklaan 12 8500 Kortrijk",
        ),
        (
            ...
            "De Korenbloem VZW",
            "foi_contact",
            "tick2341; info@dekorenbloem.net; Pieter de Conincklaan 12, 8500 Kortrijk; T 056 26 01 01",
        ),
    ]:
        append_csv(
            ROOT / "sources.csv",
...
append_csv(
    ...
    dict(
        ...
        notes="tick2341 YE2025 Medium CW NL+EN+FR + Strong KBO 0418.825.412 Actief VZW 1 VE Aanbestedende RSZ 87.101; omzet JUMP 12564145 (+2.22%); bruto JUMP 12311379 (+0.17%); pnl LOSS FLIP -360537; equity DROP 8688763 (-4.9%); FTE JUMP 175.9; neerlegging 03.06.2026; assets/debt Unknown; FOI gap_korenbloem_*; after Konekt EVERY-10@2340; AGB/FARO YE2024; Gandae/Aralea/Manupal/Vlotter YE2024; not TE-additive of 348bn",
    ),
)

for bid, amt, basis, notes in [
    ("bud_korenbloem_omzet_jr2025_statutory", 12564145, "CW statutory omzet YE2025 JUMP", "tick2341; Medium CW; omzet +2.22% vs 12291695"),
    ("bud_korenbloem_bruto_jr2025_statutory", 12311379, "CW statutory bruto_marge YE2025", "tick2341; Medium CW; bruto +0.17% vs 12290405"),
    ("bud_korenbloem_pnl_jr2025_statutory", -360537, "CW statutory winst/verlies YE2025 LOSS FLIP", "tick2341; Medium CW; pnl LOSS FLIP from +73799 YE2024"),
    ("bud_korenbloem_equity_jr2025_statutory", 8688763, "CW statutory eigen_vermogen YE2025 DROP", "tick2341; Medium CW; equity -4.9% vs 9136438"),
    ("bud_korenbloem_fte_jr2025_statutory", 175.9, "CW social-balance FTE 175.9", "tick2341; Medium CW; FTE 175.9 vs 172.7"),
]:
    append_csv(
        ROOT / "budgets.csv",
        dict(
...
append_csv(
    ...
    dict(
        ...
        status="open",
        struck_reason="",
        notes="tick2341; Medium CW; FOI gap_korenbloem_*; after Konekt EVERY-10@2340",
    ),
)
...
for row in rows:
    if row["task_id"] == "rq_2341":
        ...
        row["updated_utc"] = UTC
        row["notes"] = (
            "tick2341 De Korenbloem 0418.825.412 YE2025 Medium; omzet 12564145; bruto 12311379; "
            "pnl LOSS FLIP -360537; equity DROP 8688763; FTE 175.9; FOI ready NOT sent; after Konekt EVERY-10@2340; next EVERY-10 2350"
        )
        row["instructions"] = (
            "After Konekt EVERY-10@2340. Prefer AGB/FARO YE2025 else unused. Do NOT redo De Korenbloem/Konekt/OZC SV stack."
        )

if not any(row["task_id"] == "rq_2342" for row in rows):
    rows.append(
        {
            "task_id": "rq_2342",
            ...
            "instructions": (
                ...
                "Do NOT redo De Korenbloem/Konekt/OZC SV/De Cirkel/Wieltjesgracht/Apojo stack."
            ),
            "blocked_gap_id": "",
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2341 De Korenbloem; next EVERY-10 2350",
        }
    )
...
with open(ROOT / "loop_state.csv", "w", newline="", encoding="utf-8") as f:
    ...
    w.writerow(
        dict(
            ...
            last_tick_utc=UTC,
            last_unit_id="rq_2341",
            ticks_completed="2341",
            paused="no",
            notes=(
                "tick2341 leftover dual De Korenbloem 0418.825.412 Medium "
                "(omzet JUMP 12564145; bruto JUMP 12311379; pnl LOSS FLIP -360537; equity DROP 8688763; FTE JUMP 175.9; "
                "1 VE Kortrijk WZC RVT Aanbestedende); after Konekt EVERY-10@2340; AGB/FARO YE2024; next rq_2342; next EVERY-10 2350"
            ),
        )
    )
...
with open("docs/doge/loop_log.md", "a", encoding="utf-8") as f:
    f.write(
        f"""
### {UTC} — tick 2341 — rq_2341 De Korenbloem Kortrijk (omzet JUMP 12.56m / pnl LOSS FLIP / equity DROP / FTE 175.9 / Medium)

- Unit: **rq_2341** leftover dual after Konekt EVERY-10@2340. Stalls AGB Bornem / FARO / AIESH / Gandae / Aralea / Manupal / Vlotter still **YE2024**. Took FREE Flemish WZC **De Korenbloem VZW** YE2025 (KBO **0418.825.412**; Pieter de Conincklaan 12, 8500 Kortrijk; **Actief** **1 VE**; Aanbestedende; RSZ **87.101** RVT; info@dekorenbloem.net).
...
- Wrote: sources (+5); budgets (+5); commitments (+1); leaderboard (+1 pi 6.00); entities (+1 {ENTITY}); foi + draft `{GAP}`; rq_2341=done + rq_2342 open; loop_state ticks=2341.
- FOI: **ready not sent**. NOT every-10 (next **2350**). Next: rq_2342.
"""
    )

print("OK tick2341 De Korenbloem")
