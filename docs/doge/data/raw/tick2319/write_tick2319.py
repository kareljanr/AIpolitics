...
utc = "2026-08-27T22:50:00Z"
Path("docs/doge/data/raw/tick2319").mkdir(parents=True, exist_ok=True)
...
for row in rows:
    if row["task_id"] == "rq_2319" and row["status"] == "done":
        raise SystemExit("rq_2319 already done: " + (row.get("title") or "")[:90])
...
if not has_id("docs/doge/data/sources.csv", "source_id", "src_kindervriend_jr2025_cw_en"):
    for sid, title, url, pub, klass, notes in [
        (
            ...
            "tick2319; YE2025 omzet 406928 bruto 8083623 ~19.86x pnl LOSS -88717 equity 9787469 FTE 96.6",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            "tick2319; EN Medium; filed 22-05-2026; Turnover 406928 Gross 8083623 P/L -88717 Equity 9787469 FTE 96.6",
        ),
        (
        ...
        (
            ...
            "KBO FOD Economie",
            "official_register",
            "tick2319; Actief 3 VE mpi@kindervriend.be RSZ 87.201 Kortrijk",
        ),
        (
            "src_kindervriend_site_contact_2319",
            "De Kindervriend FOI mpi@kindervriend.be",
            "https://www.dekindervriend.be/",
            "MPI De Kindervriend VZW",
            "foi_contact",
            "tick2319; mpi@kindervriend.be; Rollegemkerkstraat 51 Kortrijk",
        ),
    ]:
        append_csv(
...
if not has_id("docs/doge/data/entities.csv", "entity_id", "vzw_mpi_de_kindervriend_kortrijk"):
    append_csv(
        ...
        dict(
            ...
            notes=(
                "tick2319 YE2025 Medium CW + Strong KBO 0409.988.514 Actief 3 VE RSZ 87.201; "
                "omzet JUMP 406928; bruto JUMP 8083623 ~19.86x; pnl LOSS -88717; equity JUMP 9787469; "
                "FTE 96.6; FOI gap_kindervriend_*; Homevil YE2024 stall remap; after Homevil@2318; not TE-additive"
            ),
        ),
    )
...
if not has_id(
    ...
    append_csv(
        ...
        dict(
            ...
            notes="tick2319; Medium CW; Homevil YE2024 stall remap; after Homevil@2318; not TE-additive",
        ),
    )
...
if not has_id(
    ...
    append_csv(
        ...
        dict(
            ...
            status="open",
            struck_reason="",
            notes="tick2319; Medium CW; FOI gap_kindervriend_*; after Homevil@2318",
        ),
    )
...
if not has_id(
    ...
    append_csv(
        ...
        dict(
            ...
            created_utc=utc,
            updated_utc=utc,
            notes="tick2319; ready NOT sent",
        ),
    )

for row in rows:
    if row["task_id"] == "rq_2319":
        row.update(
            {
                ...
                "notes": "tick2319; Kindervriend 0409.988.514 YE2025 Medium; omzet JUMP 406928; bruto JUMP 8083623 ~19.86x; pnl LOSS -88717; equity JUMP 9787469; FTE 96.6; FOI ready NOT sent; Homevil YE2024 stall; next EVERY-10 2320",
            }
        )

if not any(row["task_id"] == "rq_2320" for row in rows):
    rows.append(
        {
            "task_id": "rq_2320",
            ...
            "hierarchy_target": "L5",
            "entity_id": "",
            "instructions": "After Kindervriend. Prefer AGB/FARO YE2025 else unused (Manupal/Aralea/Vlotter/Gandae/De Ploeg/Homevil if YE2025). Do NOT redo Kindervriend/Homevil/Olo-Rotonde/Havenzate/Iris stack.",
            "blocked_gap_id": "",
            "created_utc": utc,
            "updated_utc": utc,
            "notes": "spawned after tick2319 Kindervriend; next every-10 2320",
        }
    )
    ...
    w.writerow(
        {
            ...
            "last_tick_utc": utc,
            "last_unit_id": "rq_2319",
            "ticks_completed": "2319",
            "paused": "no",
            "notes": (
                "tick2319 leftover dual MPI De Kindervriend 0409.988.514 Medium (omzet JUMP 406928; bruto JUMP 8083623 ~19.86x; "
                "pnl LOSS -88717; equity JUMP 9787469; FTE 96.6; 3 VE Kortrijk VAPH); Homevil YE2024 stall remap; "
                "after Homevil@2318; AGB Bornem JR2024; FARO/AIESH YE2024; next rq_2320; next EVERY-10 2320; continuous hole_fill"
            ),
        }
    )
