...
UTC = "2026-08-28T03:40:00Z"
ROOT = Path("docs/doge/data")
Path("docs/doge/data/raw/tick2345").mkdir(parents=True, exist_ok=True)
...
append_csv(
    ...
    dict(
        ...
        notes=f"tick{TICK} YE2025 Medium CW NL+EN+FR + Strong KBO 0418.825.412 Actief VZW 1 VE Aanbestedende RSZ 87.101; omzet JUMP 12564145 (+2.22%); bruto JUMP 12311379 (+0.17%); pnl LOSS FLIP -360537; equity DROP 8688763 (-4.9%); FTE JUMP 175.9; neerlegging 03.06.2026; assets/debt Unknown; FOI gap_korenbloem_*; after Blijdorp@2344; AGB/FARO YE2024; Gandae/Aralea/Manupal/Vlotter YE2024; not TE-additive of 348bn",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        notes=f"tick{TICK}; Medium CW; after Blijdorp@2344; not TE-additive",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        status="open",
        struck_reason="",
        notes=f"tick{TICK}; Medium CW; FOI gap_korenbloem_*; after Blijdorp@2344",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        created_utc=UTC,
        updated_utc=UTC,
        notes=f"tick{TICK}; ready NOT sent; Medium CW + Strong KBO; after Blijdorp@2344",
    ),
)
...
- After Blijdorp@2344. Stalls: AGB Bornem JR2024; FARO/AIESH/Gandae/Aralea/Manupal/Vlotter YE2024.
...
for row in rows:
    if row["task_id"] == RQ:
        ...
        row["notes"] = (
            ...
            "pnl LOSS FLIP -360537; equity DROP 8688763; FTE 175.9; FOI ready NOT sent; after Blijdorp@2344; next EVERY-10 2350"
        )
        row["instructions"] = (
            "After Blijdorp@2344. Prefer AGB/FARO YE2025 else unused. Do NOT redo De Korenbloem/Blijdorp/Perrekes/Aurelia stack."
        )
    ...
    w.writerow(
        dict(
            ...
            notes=(
                ...
                f"1 VE Kortrijk WZC RVT Aanbestedende); after Blijdorp@2344; AGB/FARO YE2024; next {RQ_NEXT}; next EVERY-10 2350"
            ),
        )
    )
...
- Unit: **{RQ}** leftover dual after Blijdorp@2344. Stalls AGB Bornem / FARO / AIESH / Gandae / Aralea / Manupal / Vlotter still **YE2024**. Took FREE Flemish WZC **De Korenbloem VZW** YE2025 (KBO **0418.825.412**; Pieter de Conincklaan 12, 8500 Kortrijk; **Actief** **1 VE**; Aanbestedende; RSZ **87.101** RVT; info@dekorenbloem.net).
