# tick 2325: leftover dual aPart Gent YE2025 after Ritmica@2324
import csv
import json
...
if not has_id(ROOT / "sources.csv", "source_id", "src_apart_jr2025_cw_en"):
    for sid, title, url, pub, klass, notes in [
        (
            ...
            f"tick2325; YE2025 omzet {OMZET} bruto {BRUTO} ~{RATIO}x pnl DROP {PNL} equity {EQUITY} FTE {FTE}",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            f"tick2325; EN Medium; filed 24-06-2026; Turnover {OMZET} Gross {BRUTO} P/L {PNL} Equity {EQUITY} FTE {FTE}",
        ),
        (
        ...
        (
            ...
            "KBO FOD Economie",
            "official_register",
            "tick2325; Actief 12 VE aPart Gent RSZ 87.991 integrale jeugdhulp",
        ),
        (
            "src_apart_site_contact_2325",
            "aPart FOI info.vzwapart@vzwapart.be",
            "https://vzwapart.be/contacteer-ons",
            "aPart VZW",
            "foi_contact",
            "tick2325; info.vzwapart@vzwapart.be; Brandstraat 3 Gent; T 09 225 01 59",
        ),
    ]:
        append_csv(
            ROOT / "sources.csv",
...
if not has_id(ROOT / "entities.csv", "entity_id", ENTITY):
    append_csv(
        ...
        dict(
            ...
            notes=(
                f"tick2325 YE2025 Medium CW NL+EN+FR + Strong KBO 0567.657.460 Actief 12 VE RSZ 87.991; "
                f"omzet DROP {OMZET}; bruto JUMP {BRUTO} ~{RATIO}x; pnl DROP {PNL} (-99.58%); equity {EQUITY}; "
                f"FTE JUMP {FTE}; FOI {GAP}; after Ritmica@2324; AGB/FARO YE2024; not TE-additive"
            ),
        ),
    )
...
if not has_id(ROOT / "budgets.csv", "budget_id", "bud_apart_omzet_jr2025_statutory"):
    for bid, amt, basis, notes in [
        ("bud_apart_omzet_jr2025_statutory", OMZET, "CW statutory omzet YE2025 DROP", f"tick2325; Medium CW; omzet -21.51% vs {OMZET24}"),
        ("bud_apart_bruto_jr2025_statutory", BRUTO, f"CW statutory bruto_marge YE2025 ~{RATIO}x omzet", f"tick2325; Medium CW; bruto +5.19% vs {BRUTO24}"),
        ("bud_apart_pnl_jr2025_statutory", PNL, "CW statutory winst/verlies YE2025 DROP -99.58%", f"tick2325; Medium CW; pnl DROP vs {PNL24}"),
        ("bud_apart_equity_jr2025_statutory", EQUITY, "CW statutory eigen_vermogen YE2025", f"tick2325; Medium CW; equity -0.27% vs {EQUITY24}"),
        ("bud_apart_fte_jr2025_statutory", FTE, "CW social-balance FTE 115.7", f"tick2325; Medium CW; FTE JUMP vs {FTE24}"),
    ]:
        append_csv(
            ROOT / "budgets.csv",
            dict(
...
if not has_id(ROOT / "leaderboard.csv", "item_id", LB):
    append_csv(
        ...
        dict(
            ...
            status="open",
            struck_reason="",
            notes=f"tick2325; Medium CW; FOI {GAP}; after Ritmica@2324; AGB/FARO YE2024",
        ),
    )
...
if not has_id(ROOT / "foi_queue.csv", "gap_id", GAP):
    append_csv(
        ...
        dict(
            ...
            created_utc=UTC,
            updated_utc=UTC,
            notes="tick2325; ready NOT sent; Medium CW + Strong KBO; after Ritmica@2324",
        ),
    )
...
for row in rows:
    if row["task_id"] == "rq_2325":
        ...
        row["instructions"] = (
            "After Ritmica. Prefer AGB/FARO YE2025 else unused. Do NOT redo Ritmica/Dominiek Savio/Merlijn/Humival/Heder stack."
        )
        row["blocked_gap_id"] = GAP
        row["updated_utc"] = UTC
        row["notes"] = (
            f"tick2325; aPart 0567.657.460 Medium CW; omzet DROP {OMZET}; bruto JUMP {BRUTO} ~{RATIO}x; "
            f"pnl DROP {PNL}; equity {EQUITY}; FTE JUMP {FTE}; FOI {GAP} ready NOT sent; after Ritmica@2324; next EVERY-10 2330"
        )
    ...
    out_rows.append(
        {
            ...
            "instructions": (
                ...
                "Do NOT redo aPart/Ritmica/Dominiek Savio/Merlijn/Humival/Heder/Kindervriend/Homevil stack."
            ),
            "blocked_gap_id": "",
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2325 aPart; next every-10 2330",
        }
    )
    ...
    w.writerow(
        {
            ...
            "paused": "no",
            "notes": (
                f"tick2325 leftover dual aPart 0567.657.460 Medium (omzet DROP {OMZET}; bruto JUMP {BRUTO} ~{RATIO}x; "
                f"pnl DROP {PNL}; equity {EQUITY}; FTE JUMP {FTE}; 12 VE Gent jeugdhulp); after Ritmica@2324; "
                "AGB Bornem JR2024; FARO/AIESH YE2024; next rq_2326; next EVERY-10 2330; continuous hole_fill"
            ),
        }
    )

raw = ROOT / "raw" / "tick2325"
