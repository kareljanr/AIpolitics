...
for name, url in [
    ("prinsen_en.html", f"https://www.companyweb.be/en/{kbo}"),
    ("prinsen_nl.html", f"https://www.companyweb.be/nl/{kbo}"),
    ("prinsen_fr.html", f"https://www.companyweb.be/fr/{kbo}"),
    (
        "prinsen_kbo.html",
...
t = (out / "prinsen_en.html").read_text(encoding="utf-8", errors="ignore")
...
k = (out / "prinsen_kbo.html").read_text(encoding="utf-8", errors="ignore")
kp = re.sub(r"<[^>]+>", " ", k)
kp = re.sub(r"\s+", " ", kp)
for key in [
    "Status",
...
# site
for name, url in [
    ("prinsen_site.html", "https://www.prinsenhof.be/"),
    ("prinsen_site2.html", "https://www.wzcprinsenhof.be/"),
]:
    try:
        t2 = fetch(url, out / name)
        title2 = re.search(r"<title>([^<]+)", t2)
        print("SITE", name, len(t2), title2.group(1)[:80] if title2 else "")
