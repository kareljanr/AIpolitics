...
for name, url in [
    ("mbn_en.html", "https://www.companyweb.be/en/0415653116"),
    ("mbn_nl.html", "https://www.companyweb.be/nl/0415653116"),
    ("mbn_fr.html", "https://www.companyweb.be/fr/0415653116"),
    ("mbn_kbo.html", "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?lang=nl&ondernemingsnummer=0415653116"),
]:
    t = fetch(url, out / name)
    print("OK", name, len(t))

t = (out / "mbn_en.html").read_text(encoding="utf-8", errors="ignore")
...
k = (out / "mbn_kbo.html").read_text(encoding="utf-8", errors="ignore")
kp = re.sub(r"<[^>]+>", " ", k)
kp = re.sub(r"\s+", " ", kp)
for key in ["Status", "Actief", "Adres", "Niel", "vestigingseenheden", "E-mail", "Telefoon", "Aanbestedende", "NACE", "87.", "Rechtsvorm", "Naam"]:
