...
UTC = "2026-08-28T01:40:00Z"
ROOT = Path("docs/doge/data")
Path("docs/doge/data/raw/tick2333").mkdir(parents=True, exist_ok=True)
...
with open(path_rq, newline="", encoding="utf-8") as f:
    ...
    rq_rows = list(r)
rq = next((row for row in rq_rows if row["task_id"] == "rq_2333"), None)
if not rq or rq.get("status") not in ("open", "in_progress"):
    raise SystemExit("rq_2333 not claimable: " + repr(rq and rq.get("status")))
...
if not has_id(ROOT / "sources.csv", "source_id", "src_gielsbos_jr2025_cw_en"):
    for sid, title, url, pub, klass, notes in [
        (
            ...
            "tick2333; YE2025 omzet 5351872 bruto 41720205 ~7.80x pnl JUMP 1613061 equity 38121725 FTE 509.3",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            "tick2333; EN Medium; filed 13-06-2026; Turnover 5351872 Gross 41720205 P/L 1613061 Equity 38121725 FTE 509.3",
        ),
        (
            ...
            "KBO FOD Economie",
            "official_register",
            "tick2333; Actief VZW 7 VE Lille Aanbestedende; RSZ 87.202; BTW 87.201+87.202",
        ),
        (
            ...
            "Het GielsBos VZW",
            "foi_contact",
            "tick2333; info@hetgielsbos.be; Vosselaarseweg 1, 2275 Lille; T +32 14 60 12 11",
        ),
    ]:
        append_csv(
            ROOT / "sources.csv",
...
append_csv(
    ...
    dict(
        ...
        notes="tick2333 YE2025 Medium CW NL+EN + Strong KBO 0408.318.233 Actief VZW 7 VE Aanbestedende RSZ 87.202; omzet JUMP 5351872 (+3.71%); bruto JUMP 41720205 (~7.80x / +2.13%); pnl JUMP 1613061 (+14.35%); equity JUMP 38121725 (+4.04%); FTE 509.3; neerlegging 13.06.2026; assets/debt Unknown; FOI gap_gielsbos_*; after Alma@2332; AGB Bornem JR2024; FARO/AIESH/Gandae/Aralea/Manupal/Vlotter YE2024; not TE-additive of 348bn",
    ),
)

for bid, amt, basis, notes in [
    ("bud_gielsbos_omzet_jr2025_statutory", 5351872, "CW statutory omzet YE2025 JUMP", "tick2333; Medium CW; omzet +3.71% vs 5160465"),
    ("bud_gielsbos_bruto_jr2025_statutory", 41720205, "CW statutory bruto_marge YE2025 ~7.80x omzet", "tick2333; Medium CW; bruto +2.13% vs 40850817"),
    ("bud_gielsbos_pnl_jr2025_statutory", 1613061, "CW statutory winst/verlies YE2025 JUMP", "tick2333; Medium CW; pnl +14.35% vs 1410671"),
    ("bud_gielsbos_equity_jr2025_statutory", 38121725, "CW statutory eigen_vermogen YE2025 JUMP", "tick2333; Medium CW; equity +4.04% vs 36640735"),
    ("bud_gielsbos_fte_jr2025_statutory", 509.3, "CW social-balance FTE 509.3", "tick2333; Medium CW; FTE 509.3 vs 510.7"),
]:
    append_csv(
        ROOT / "budgets.csv",
        dict(
...
append_csv(
    ...
    dict(
        ...
        notes="tick2333; Medium CW; after Alma@2332; not TE-additive",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        status="open",
        struck_reason="",
        notes="tick2333; Medium CW; FOI gap_gielsbos_*; after Alma@2332",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        created_utc=UTC,
        updated_utc=UTC,
        notes="tick2333; ready NOT sent; Medium CW + Strong KBO; after Alma@2332",
    ),
)
...
for row in rows:
    if row["task_id"] == "rq_2333":
        ...
        row["updated_utc"] = UTC
        row["notes"] = (
            "tick2333 Het GielsBos 0408.318.233 YE2025 Medium; omzet 5351872; bruto 41720205 ~7.80x; "
            "pnl JUMP 1613061; equity 38121725; FTE 509.3; FOI ready NOT sent; after Alma@2332; next EVERY-10 2340"
        )
        row["instructions"] = (
            "After Den Brand EVERY-10@2330. Prefer AGB/FARO YE2025 else unused. Do NOT redo Het GielsBos/Mivalti/Den Brand/Tandem stack."
        )

if not any(row["task_id"] == "rq_2334" for row in rows):
    rows.append(
        {
            "task_id": "rq_2334",
            ...
            "entity_id": "",
            "instructions": (
                "After Het GielsBos YE2025 Medium@2333. Prefer AGB/FARO YE2025 else FREE "
            ...
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2333 Het GielsBos; next EVERY-10 2340",
        }
    )
...
with open(ROOT / "loop_state.csv", "w", newline="", encoding="utf-8") as f:
    ...
    w.writerow(
        dict(
            ...
            last_tick_utc=UTC,
            last_unit_id="rq_2333",
            ticks_completed="2333",
            paused="no",
            notes=(
                "tick2333 leftover dual Het GielsBos 0408.318.233 Medium "
                "(omzet JUMP 5351872; bruto JUMP 41720205 ~7.80x; pnl JUMP 1613061; equity JUMP 38121725; FTE 509.3; "
                "7 VE Lille VAPH Aanbestedende); after Alma@2332; AGB/FARO YE2024; next rq_2334; next EVERY-10 2340"
            ),
        )
    )
...
with open("docs/doge/loop_log.md", "a", encoding="utf-8") as f:
    f.write(
        f"""
### {UTC} - tick 2333 - rq_2333 Het GielsBos Lille (bruto JUMP 41.72m / ~7.80x omzet / pnl JUMP / FTE 509.3 / Medium)

- Unit: **rq_2333** leftover dual after Alma@2332. Stalls AGB Bornem / FARO / AIESH / Gandae / Aralea / Manupal / Vlotter still **YE2024**. Took FREE Flemish VAPH **Het GielsBos VZW** YE2025 (KBO **0408.318.233**; Vosselaarseweg 1, 2275 Lille; **Actief** **7 VE**; Aanbestedende; RSZ **87.202**; info@hetgielsbos.be).
...
- Wrote: sources (+4); budgets (+5); commitments (+1); leaderboard (+1 pi 6.00); entities (+1 {ENTITY}); foi + draft `{GAP}`; rq_2333=done + rq_2334 open; loop_state ticks=2333.
- FOI: **ready not sent**. NOT every-10 (next **2340**). Next: rq_2333.
"""
    )

print("OK tick2333 Het GielsBos")
