...
for row in rows:
    if row["task_id"] == "rq_2329" and row["status"] == "done":
        raise SystemExit("rq_2329 already done: " + (row.get("title") or "")[:90])

if not has_id("docs/doge/data/sources.csv", "source_id", "src_mivalti_jr2025_cw_en"):
    for sid, title, url, pub, klass, notes in [
        ("src_mivalti_jr2025_cw_nl", "Companyweb NL Mivalti YE2025 statutory", "https://www.companyweb.be/nl/0416406548/mivalti", "Companyweb (NBB-derived)", "secondary_aggregator", "tick2329; YE2025 omzet 1683253 bruto 11487628 ~6.82x pnl JUMP 470626 equity 8134829 FTE 134.9"),
        ("src_mivalti_jr2025_cw_en", "Companyweb EN Mivalti YE2025 statutory", "https://www.companyweb.be/en/0416406548/mivalti", "Companyweb (NBB-derived)", "secondary_aggregator", "tick2329; EN Medium; filed 16-06-2026; Turnover 1683253 Gross 11487628 P/L 470626 Equity 8134829 FTE 134.9"),
        ("src_mivalti_jr2025_cw_fr", "Companyweb FR Mivalti YE2025 statutory", "https://www.companyweb.be/fr/0416406548/mivalti", "Companyweb (NBB-derived)", "secondary_aggregator", "tick2329; FR mirror"),
        ("src_mivalti_kbo_2329", "KBO Mivalti 0416.406.548", "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?ondernemingsnummer=0416406548", "KBO FOD Economie", "official_register", "tick2329; Actief 2 VE Tielt RSZ 87.202"),
        ("src_mivalti_site_contact_2329", "Mivalti FOI info@mivalti.be", "https://www.mivalti.be/", "Mivalti VZW", "foi_contact", "tick2329; info@mivalti.be; Gruuthusestraat 36 Tielt"),
    ]:
...
if not has_id("docs/doge/data/entities.csv", "entity_id", "vzw_mivalti_tielt"):
    append_csv(
        ...
        dict(
            ...
            notes="tick2329 YE2025 Medium CW + Strong KBO 0416.406.548 Actief 2 VE RSZ 87.202; omzet JUMP 1683253; bruto JUMP 11487628 ~6.82x; pnl JUMP 470626; equity JUMP 8134829; FTE 134.9; FOI gap_mivalti_*; after HetEepos@2328; not TE-additive",
        ),
    )

if not has_id("docs/doge/data/budgets.csv", "budget_id", "bud_mivalti_omzet_jr2025_statutory"):
    for bid, amt, basis, notes in [
        ("bud_mivalti_omzet_jr2025_statutory", 1683253, "CW statutory omzet YE2025 JUMP", "tick2329; Medium CW; omzet +4.55% vs 1609944"),
        ("bud_mivalti_bruto_jr2025_statutory", 11487628, "CW statutory bruto_marge YE2025 ~6.82x omzet", "tick2329; Medium CW; bruto +8% vs 10636440"),
        ("bud_mivalti_pnl_jr2025_statutory", 470626, "CW statutory winst/verlies YE2025 JUMP", "tick2329; Medium CW; pnl +26.88% vs 370928"),
        ("bud_mivalti_equity_jr2025_statutory", 8134829, "CW statutory eigen_vermogen YE2025 JUMP", "tick2329; Medium CW; equity +6.19% vs 7660758"),
        ("bud_mivalti_fte_jr2025_statutory", 134.9, "CW social-balance FTE 134.9", "tick2329; Medium CW; FTE 134.9 vs 133.1"),
    ]:
        append_csv(
...
if not has_id("docs/doge/data/commitments.csv", "commitment_id", "comm_mivalti_jr2025_statutory_vaph_bruto_11_49m_6_82x"):
    append_csv(
        ...
        dict(
            ...
            notes="tick2329; Medium CW; after HetEepos@2328; not TE-additive",
        ),
    )
...
if not has_id("docs/doge/data/leaderboard.csv", "item_id", "lb_mivalti_bruto_11_49m_gt_omzet_6_82x_pnl_jump_jr2025"):
    append_csv(
        ...
        dict(
            ...
            status="open",
            struck_reason="",
            notes="tick2329; Medium CW; FOI gap_mivalti_*; after HetEepos@2328",
        ),
    )
...
if not has_id("docs/doge/data/foi_queue.csv", "gap_id", "gap_mivalti_nbb_pdf_assets_debt_bruto_gt_omzet_6_82x_pnl_jump_vaph_matrix_l5"):
    append_csv(
        ...
        dict(
            ...
            created_utc=utc,
            updated_utc=utc,
            notes="tick2329; ready NOT sent",
        ),
    )

for row in rows:
    if row["task_id"] == "rq_2329":
        row.update(
            {
                ...
                "updated_utc": utc,
                "instructions": "leftover dual Mivalti VAPH YE2025 after HetEepos@2328; Het Eepos CW N/A stall",
                "notes": "tick2329; Mivalti 0416.406.548 YE2025 Medium; omzet JUMP 1683253; bruto JUMP 11487628 ~6.82x; pnl JUMP 470626; equity JUMP 8134829; FTE 134.9; FOI ready NOT sent; next EVERY-10 2330",
            }
        )
    ...
    rows.append(
        {
            ...
            "hierarchy_target": "L5",
            "entity_id": "",
            "instructions": "After Mivalti. Prefer AGB/FARO YE2025 else unused (Manupal/Aralea/Vlotter/Gandae/Alma if YE2025). Do NOT redo Mivalti/HetEepos/Pleegzorg/Zonnebeke/Ithaka stack.",
            "blocked_gap_id": "",
            "created_utc": utc,
            "updated_utc": utc,
            "notes": "spawned after tick2329 Mivalti; next every-10 2330",
        }
    )
    ...
    w.writerow(
        {
            ...
            "last_tick_utc": utc,
            "last_unit_id": "rq_2329",
            "ticks_completed": "2329",
            "paused": "no",
            "notes": "tick2329 leftover dual Mivalti 0416.406.548 Medium (omzet JUMP 1683253; bruto JUMP 11487628 ~6.82x; pnl JUMP 470626; equity JUMP 8134829; FTE 134.9; 2 VE Tielt VAPH); after HetEepos@2328; AGB Bornem JR2024; FARO/AIESH YE2024; next rq_2326; next EVERY-10 2330",
        }
    )
