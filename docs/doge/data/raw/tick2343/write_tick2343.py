...
UTC = "2026-08-28T03:20:00Z"
ROOT = Path("docs/doge/data")
Path("docs/doge/data/raw/tick2343").mkdir(parents=True, exist_ok=True)
...
if not has_id(ROOT / "sources.csv", "source_id", "src_korenbloem_jr2025_cw_en"):
    for sid, title, url, pub, klass, notes in [
        (
            ...
            "tick2343; YE2025 omzet JUMP 12564145 bruto JUMP 12311379 pnl LOSS FLIP -360537 equity DROP 8688763 FTE JUMP 175.9; filed 03-06-2026",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            "tick2343; EN Medium; Turnover 12564145 Gross 12311379 P/L -360537 Equity 8688763 FTE 175.9; filed 03-06-2026",
        ),
        (
            ...
            "Companyweb (NBB-derived)",
            "secondary_aggregator",
            "tick2343; FR mirror YE2025 CA 12564145 marge 12311379 perte -360537 equity 8688763 FTE 175.9",
        ),
        (
            "src_korenbloem_kbo_2343",
            "KBO De Korenbloem 0418.825.412",
            "https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?ondernemingsnummer=0418825412",
            "KBO FOD Economie",
            "official_register",
            "tick2343; Actief VZW 1 VE Kortrijk Aanbestedende; RSZ 87.101 RVT; zetel Pieter de Conincklaan 12 8500 Kortrijk",
        ),
        (
            "src_korenbloem_site_contact_2343",
            "De Korenbloem FOI info@dekorenbloem.net",
            "https://www.dekorenbloem.net/",
            "De Korenbloem VZW",
            "foi_contact",
            "tick2343; info@dekorenbloem.net; Pieter de Conincklaan 12, 8500 Kortrijk; T 056 26 01 01",
        ),
    ]:
        append_csv(
            ROOT / "sources.csv",
...
append_csv(
    ...
    dict(
        ...
        notes="tick2343 YE2025 Medium CW NL+EN+FR + Strong KBO 0418.825.412 Actief VZW 1 VE Aanbestedende RSZ 87.101; omzet JUMP 12564145 (+2.22%); bruto JUMP 12311379 (+0.17%); pnl LOSS FLIP -360537; equity DROP 8688763 (-4.9%); FTE JUMP 175.9; neerlegging 03.06.2026; assets/debt Unknown; FOI gap_korenbloem_*; after Aurelia@2342; AGB/FARO YE2024; Gandae/Aralea/Manupal/Vlotter YE2024; not TE-additive of 348bn",
    ),
)

for bid, amt, basis, notes in [
    ("bud_korenbloem_omzet_jr2025_statutory", 12564145, "CW statutory omzet YE2025 JUMP", "tick2343; Medium CW; omzet +2.22% vs 12291695"),
    ("bud_korenbloem_bruto_jr2025_statutory", 12311379, "CW statutory bruto_marge YE2025", "tick2343; Medium CW; bruto +0.17% vs 12290405"),
    ("bud_korenbloem_pnl_jr2025_statutory", -360537, "CW statutory winst/verlies YE2025 LOSS FLIP", "tick2343; Medium CW; pnl LOSS FLIP from +73799 YE2024"),
    ("bud_korenbloem_equity_jr2025_statutory", 8688763, "CW statutory eigen_vermogen YE2025 DROP", "tick2343; Medium CW; equity -4.9% vs 9136438"),
    ("bud_korenbloem_fte_jr2025_statutory", 175.9, "CW social-balance FTE 175.9", "tick2343; Medium CW; FTE 175.9 vs 172.7"),
]:
    append_csv(
        ROOT / "budgets.csv",
        dict(
...
append_csv(
    ...
    dict(
        ...
        notes="tick2343; Medium CW; after Aurelia@2342; not TE-additive",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        status="open",
        struck_reason="",
        notes="tick2343; Medium CW; FOI gap_korenbloem_*; after Aurelia@2342",
    ),
)
...
append_csv(
    ...
    dict(
        ...
        created_utc=UTC,
        updated_utc=UTC,
        notes="tick2343; ready NOT sent; Medium CW + Strong KBO; after Aurelia@2342",
    ),
)
...
- After Aurelia@2342. Stalls: AGB Bornem JR2024; FARO/AIESH/Gandae/Aralea/Manupal/Vlotter YE2024.
...
for row in rows:
    if row["task_id"] == "rq_2343":
        ...
        row["updated_utc"] = UTC
        row["notes"] = (
            "tick2343 De Korenbloem 0418.825.412 YE2025 Medium; omzet 12564145; bruto 12311379; "
            "pnl LOSS FLIP -360537; equity DROP 8688763; FTE 175.9; FOI ready NOT sent; after Aurelia@2342; next EVERY-10 2350"
        )
        row["instructions"] = (
            "After Aurelia@2342. Prefer AGB/FARO YE2025 else unused. Do NOT redo De Korenbloem/Aurelia/Eyckerheyde/Konekt stack."
        )

if not any(row["task_id"] == "rq_2344" for row in rows):
    rows.append(
        {
            "task_id": "rq_2344",
            ...
            "entity_id": "",
            "instructions": (
                "After De Korenbloem YE2025 Medium@2343. Prefer AGB/FARO YE2025 else FREE "
                "(Gandae/Aralea/Manupal/Vlotter if YE2025 / unused ETA-VAPH-WZC). "
                "Do NOT redo De Korenbloem/Aurelia/Huize Eyckerheyde/Konekt/OZC SV stack."
            ),
            "blocked_gap_id": "",
            "created_utc": UTC,
            "updated_utc": UTC,
            "notes": "spawned after tick2343 De Korenbloem; next EVERY-10 2350",
        }
    )
...
with open(ROOT / "loop_state.csv", "w", newline="", encoding="utf-8") as f:
    ...
    w.writerow(
        dict(
            ...
            ticks_completed="2343",
            paused="no",
            notes=(
                "tick2343 leftover dual De Korenbloem 0418.825.412 Medium "
                "(omzet JUMP 12564145; bruto JUMP 12311379; pnl LOSS FLIP -360537; equity DROP 8688763; FTE JUMP 175.9; "
                "1 VE Kortrijk WZC RVT Aanbestedende); after Aurelia@2342; AGB/FARO YE2024; next rq_2344; next EVERY-10 2350"
            ),
        )
    )
...
with open("docs/doge/loop_log.md", "a", encoding="utf-8") as f:
    f.write(
        f"""
### {UTC} — tick 2343 — rq_2343 De Korenbloem Kortrijk (omzet JUMP 12.56m / pnl LOSS FLIP / equity DROP / FTE 175.9 / Medium)

- Unit: **rq_2343** leftover dual after Aurelia@2342. Stalls AGB Bornem / FARO / AIESH / Gandae / Aralea / Manupal / Vlotter still **YE2024**. Took FREE Flemish WZC **De Korenbloem VZW** YE2025 (KBO **0418.825.412**; Pieter de Conincklaan 12, 8500 Kortrijk; **Actief** **1 VE**; Aanbestedende; RSZ **87.101** RVT; info@dekorenbloem.net).
...
- Wrote: sources (+5); budgets (+5); commitments (+1); leaderboard (+1 pi 6.00); entities (+1 {ENTITY}); foi + draft `{GAP}`; rq_2343=done + rq_2344 open; loop_state ticks=2343.
- FOI: **ready not sent**. NOT every-10 (next **2350**). Next: rq_2344.
"""
    )

print("OK tick2343 De Korenbloem")
