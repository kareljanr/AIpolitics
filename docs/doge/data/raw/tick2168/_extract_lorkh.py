...
for name, url in [
    ("lorkh_en.html", f"https://www.companyweb.be/en/{kbo}"),
    ("lorkh_nl.html", f"https://www.companyweb.be/nl/{kbo}"),
    ("lorkh_fr.html", f"https://www.companyweb.be/fr/{kbo}"),
    (
        "lorkh_kbo.html",
        f"https://kbopub.economie.fgov.be/kbopub/toonondernemingps.html?lang=nl&ondernemingsnummer={kbo}",
    ),
    ("foyer_site.html", "https://www.foyerdelork.be/"),
    ("careprop.html", "https://carepropertyinvest.be/"),
]:
    try:
...
for site in ["foyer_site.html", "careprop.html"]:
    p = out / site
    if p.exists() and p.stat().st_size > 100:
        s = p.read_text(encoding="utf-8", errors="ignore")
        emails = re.findall(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", s, re.I)
        print(site, "emails", list(dict.fromkeys(emails))[:8])
