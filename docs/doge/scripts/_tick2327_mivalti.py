# -*- coding: utf-8 -*-
"""Tick 2327: Mivalti Tielt VAPH YE2025 — APPEND-ONLY."""
...
RAW = DATA / "raw" / "tick2327"
FOI_DRAFTS = ROOT / "docs" / "doge" / "foi" / "drafts"
LOG = ROOT / "docs" / "doge" / "loop_log.md"

UTC = "2026-08-28T00:40:00Z"
TICK = "2327"
RQ, NEXT_RQ = "rq_2327", "rq_2328"
...
print("sources +", append_rows(DATA / "sources.csv", "source_id", [
    ...
    {"source_id": "src_mivalti_site_contact_2327", "title": "Mivalti FOI info@mivalti.be", "url": "https://mivalti.be/", "publisher": "Mivalti VZW", "accessed_date": "2026-08-28", "source_class": "foi_contact", "notes": f"tick{TICK}; {EMAIL}; {ADDR}"},
]))
...
print("commitments +", append_rows(DATA / "commitments.csv", "commitment_id", [{
    ...
    "notes": f"tick{TICK}; Medium CW; bruto primary {BRUTO}; after Zonnebeke Sint-Jozef@2326; Het Eepos Ver.OCMW no CW YE2025",
}]))
...
print("leaderboard +", append_rows(DATA / "leaderboard.csv", "item_id", [{
    ...
    "status": "open",
    "struck_reason": "",
    "notes": f"tick{TICK}; Medium CW; FOI {GAP}; after Zonnebeke Sint-Jozef@2326",
}]))
...
print("entities +", append_rows(DATA / "entities.csv", "entity_id", [{
    ...
    "foi_email": EMAIL,
    "foi_postal": ADDR,
    "notes": f"tick{TICK} YE2025 Medium CW NL+EN+FR + Strong KBO {KBO} Actief VZW RSZ 87.202; omzet JUMP {OMZET} bruto JUMP {BRUTO} (~{RATIO}x) pnl JUMP {PNL} equity JUMP {EQUITY} FTE JUMP {FTE}; neerlegging {FILED}; FOI {GAP}; after Zonnebeke Sint-Jozef@2326; AGB Bornem JR2024; FARO/AIESH YE2024; not TE-additive",
}]))
...
for r in rqrows:
    if r.get("task_id") == RQ:
        ...
        r["notes"] = f"tick{TICK}: Mivalti YE2025 Medium (omzet JUMP {OMZET}; bruto JUMP {BRUTO} ~{RATIO}x; pnl JUMP {PNL}; equity JUMP {EQUITY}; FTE JUMP {FTE}; Tielt VAPH); FOI {GAP} ready not sent; after Zonnebeke Sint-Jozef@2326; Het Eepos Ver.OCMW no CW YE2025; stalls YE2024; next EVERY-10 2330"
if not any(r.get("task_id") == NEXT_RQ for r in rqrows):
    rqrows.append({
        ...
        "hierarchy_target": "L5",
        "entity_id": "",
        "instructions": f"After Mivalti YE2025 Medium (bruto ~{RATIO}x). Prefer AGB Bornem/APB → FARO/AIESH if YE2025 → FREE ETA-VAPH-WZC-maatwerk (Den Brand/Gandae/Manupal/Aralea if YE2025). Do NOT redo Mivalti/Zonnebeke/Ithaka/Het Eepos/Ritmica/Dominiek Savio/Humival stack.",
        "blocked_gap_id": "",
        "created_utc": UTC,
        "updated_utc": UTC,
...
for r in lsrows:
    if r.get("state_id") == "main":
        r.update({
            ...
            "paused": "no",
            "notes": f"tick{TICK} leftover dual Mivalti {KBO} Medium (omzet JUMP {OMZET}; bruto JUMP {BRUTO} ~{RATIO}x; pnl JUMP {PNL}; equity JUMP {EQUITY}; FTE JUMP {FTE}; Tielt VAPH); after Zonnebeke Sint-Jozef@2326; AGB Bornem JR2024; FARO/AIESH YE2024; next {NEXT_RQ}; next EVERY-10 2330; continuous hole_fill",
        })
