...
RAW = DATA / "raw" / "tick2330"
FOI_DRAFTS = ROOT / "docs" / "doge" / "foi" / "drafts"
LOG = ROOT / "docs" / "doge" / "loop_log.md"
UTC="2026-08-28T00:55:00Z"; TICK="2330"; RQ="rq_2330"; NEXT_RQ="rq_2331"
...
if ticks < 2329: raise SystemExit(f"unexpected ticks={ticks}")
...
NEW_TICKS="2330"; RAW.mkdir(parents=True, exist_ok=True)
...
for r in rqrows:
    if r.get("task_id")==RQ:
        ...
        r["title"]=f"EVERY-10 + leftover dual — Alma YE2025 Medium (omzet JUMP 11.24m / pnl DROP {PNL_PCT}% / FTE 96.2)"
        r["updated_utc"]=UTC
...
{"source_id":"src_alma_site_foi_2330","title":"Alma FOI ikhebeenvraag@alma.kuleuven.be","url":"https://www.alma.be/nl/contact","publisher":"Alma vzw","accessed_date":"2026-08-28","source_class":"foi_contact","notes":f"tick{TICK}; {EMAIL}"},
...
(DATA/"progress_every_10_ticks.md").write_text(f"""# DOGE progress — every 10 ticks
...
""", encoding="utf-8")
(DATA/"doge_waste_top10_current.md").write_text(f"""# DOGE waste ranking — current top 10
...
for r in rqrows:
    if r.get("task_id")==RQ:
        ...
        r["notes"]=f"tick{TICK} EVERY-10: Alma {KBO} YE2025 Medium; omzet JUMP {OMZET}; bruto JUMP {BRUTO}; pnl DROP {PNL} ({PNL_PCT}%); equity JUMP {EQUITY}; FTE JUMP {FTE}; FOI {GAP} ready NOT sent; progress+waste refreshed; after Tandem@2329; next EVERY-10 2340"
if not have_next:
    rqrows.append({"task_id":NEXT_RQ,"title":"leftover dual after Alma EVERY-10 — prefer AGB/FARO-YE2025/AIESH/or-unused","sprint":"hole_fill","priority":"8","status":"open","hierarchy_target":"L5","entity_id":"","instructions":"After Alma YE2025 Medium (omzet 11.24m / pnl DROP -13%). Prefer AGB/FARO if YE2025 else FREE (Gandae/Aralea/Manupal/Vlotter/De Ploeg if YE2025). Do NOT redo Alma/Tandem/Eepos/Pleegzorg/Dominiek/Ritmica stack.","blocked_gap_id":"","created_utc":UTC,"updated_utc":UTC,"notes":f"spawned after tick{TICK} Alma EVERY-10; next EVERY-10 2340"})
write_csv(DATA/"research_queue.csv", rq_fields, rqrows)
for r in lsrows:
    if r.get("state_id")=="main":
        r.update({"mode":"continuous","current_sprint":"hole_fill","last_tick_utc":UTC,"last_unit_id":RQ,"ticks_completed":NEW_TICKS,"paused":"no","notes":f"tick{TICK} EVERY-10 leftover dual Alma {KBO} Medium (omzet JUMP {OMZET}; bruto JUMP {BRUTO}; pnl DROP {PNL} {PNL_PCT}%; equity JUMP {EQUITY}; FTE JUMP {FTE}; Leuven Stuvo path); after Tandem@2329; AGB/FARO YE2024; next {NEXT_RQ}; next EVERY-10 2340"})
...
- Unit: **{RQ}** EVERY-10 + leftover dual after **Tandem@2329**. Stalls AGB/FARO YE2024. Took FREE **Alma VZW** YE2025 (KBO **{KBO}**; Stuvo path).
...
- FOI ready not sent. **EVERY-10 @ 2330** (next **2340**). Next: {NEXT_RQ}.
"""
