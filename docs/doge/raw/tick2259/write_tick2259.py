...
GAP = "gap_erables_nbb_pdf_assets_debt_bruto_gt_omzet_1_45x_pnl_drop_89pct_eta_matrix_l5"
COMM = "comm_erables_jr2025_statutory_eta_bruto_gt_omzet_pnl_drop_89pct"
LB = "lb_erables_omzet_4_63m_pnl_drop_89pct_jr2025"
RQ = "rq_2259"
RQ_NEXT = "rq_2260"
    ...
    for r in rows:
        if r["task_id"] == RQ:
            ...
            r["title"] = (
                ...
                f"(omzet 4.63m / pnl DROP {PNL_PCT}% / bruto~{RATIO}x / FTE {FTE})"
            )
            r["entity_id"] = ENTITY
            ...
            r["notes"] = (
                ...
                f"FTE {FTE}; 1 VE; NACE 88.993; neerlegging 09.07.2026; assets/debt Unknown; "
                f"FOI {GAP} ready NOT sent; stalls AGB Bornem JR2024 / FARO/AIESH/REW YE2024; "
                f"Relais Haute Sambre YE2024; after Val du Geer@2258; next EVERY-10 2260 MUST refresh"
            )
            r["instructions"] = (
        ...
        rows.append(
            {
                "task_id": RQ_NEXT,
                "title": (
                    "EVERY-10 + leftover dual after Les Erables — prefer AGB/FARO-YE2025/AIESH-REW/"
                ...
                "entity_id": "",
                "instructions": (
                    "EVERY-10: refresh progress_every_10_ticks.md + doge_waste_top10_current.md. "
                    "Then leftover dual after Les Erables YE2025 Medium "
                    f"(omzet 4.63m / pnl DROP {PNL_PCT}% / FTE {FTE}). "
                    ...
                    "else unused ETA/VAPH/WZC/maatwerk (e.g. Alteria / Stallbois / AJR / La Lorraine "
                    "if YE2025 FREE; skip Les Erables/Val du Geer/Nekto/Belair/Corelap/Cambier/Gaillettes/"
                ...
                "updated_utc": UTC,
                "notes": (
                    f"spawned after tick{TICK} Les Erables; EVERY-10@2260 required; "
                    "FARO/AIESH/REW YE2024; AGB Bornem JR2024; Relais Haute Sambre YE2024; next every-10 2270"
                ),
            }
        )
    ...
    rows[0].update(
        {
            ...
            "paused": "no",
            "notes": (
                f"tick{TICK} leftover Les Erables/C.A.V.A. {KBO} Medium (omzet DROP {OMZET}; "
                f"pnl DROP {PNL} {PNL_PCT}%; bruto {BRUTO} ~{RATIO}x; equity DROP {EQUITY}; FTE {FTE}; 1 VE Tournai ETA); "
                f"after Val du Geer@2258; AGB Bornem JR2024; FARO/AIESH/REW YE2024; Relais Haute Sambre YE2024; "
                f"next {RQ_NEXT} EVERY-10; next EVERY-10 2270; continuous hole_fill"
            ),
        }
    )
    with path.open("w", newline="", encoding="utf-8") as f:
...
def write_foi_draft():
    ...
    (FOI / f"{GAP}.md").write_text(
        f"""# FOI draft — Les Erables / C.A.V.A. (NBB PDF / pnl DROP {PNL_PCT}% / bruto≫omzet ~{RATIO}x)

**gap_id:** `{GAP}`  
**status:** ready (NOT sent)  
**entity:** Les Erables ASBL (C.A.V.A.) — KBO **{KBO}** (Actief; Rue du Bois des Hospices 5, 7522 Tournai; **1 VE**; FTE {FTE} CW; NACE **88.993**; Walloon ETA Tournai/Blandain)  
**recipient:** info@leserables.be · jobs@leserables.be · Rue du Bois des Hospices 5, 7522 Tournai  
**sources:** [CW EN](https://www.companyweb.be/en/{KBO_BARE}) · [CW NL](https://www.companyweb.be/nl/{KBO_BARE}) · [CW FR](https://www.companyweb.be/fr/{KBO_BARE}) · [KBO](https://kbopub.economie.fgov.be/kbopub/zoeknummerform.html?lang=nl&nummer={KBO_BARE}) · [site](https://www.leserables.be/fr/)  
**tick:** {TICK}  
**confidence:** Medium (Strong KBO + Medium CW YE2025; assets/debt Unknown)

## Context
- KBO Strong: Actief VZW/ASBL Les Erables (afkorting C.A.V.A.); **1 VE**; zetel Tournai/Blandain; NACE **88.993** (+88.995); replaces 0445.138.938.
- CW YE2025: omzet **EUR{OMZET:,}** DROP {OMZET_PCT}% vs YE2024 EUR{OMZET24:,}; bruto **EUR{BRUTO:,}** DROP {BRUTO_PCT}% (bruto≫omzet ~{RATIO}x); pnl **EUR{PNL:,}** DROP {PNL_PCT}% vs YE2024 EUR{PNL24:,}; equity **EUR{EQUITY:,}** DROP {EQUITY_PCT}%; FTE **{FTE}**; filed **09.07.2026**.
- Preferred stall: AGB Bornem JR2024; FARO/AIESH/REW YE2024; Relais Haute Sambre YE2024. After Val du Geer@2258.

## Brief
```text
[Nom] [Adresse] [E-mail] [Date]
A: Les Erables ASBL (C.A.V.A.)
via info@leserables.be
...
5. Répartition coûts isolation/calorifuge / textile / conditionnement.
...
## Tick {TICK} - {UTC} - rq_2259 Les Erables Tournai (omzet 4.63m / pnl DROP {PNL_PCT}% / bruto~{RATIO}x / FTE {FTE} / Medium)

- Unit: **rq_2259** leftover dual after **rq_2258 Val du Geer**. Prefer NON-stall live: AGB Bornem still **JR2024-only**; FARO still **YE2024**; AIESH still **YE2024**; REW still **YE2024**; Heropbeuring still **CW opaque**; Relais Haute Sambre still **YE2024**. Took named FREE Walloon ETA **Les Erables ASBL (C.A.V.A.)** YE2025 (KBO **{KBO}**; Rue du Bois des Hospices 5 Tournai; **Actief** **1 VE**; NACE **88.993** AViQ). Do not redo Val du Geer/Nekto/Belair/Corelap stack.
- Found: Companyweb NL+EN+FR YE2025 - omzet **EUR{OMZET}** DROP {OMZET_PCT}% vs YE2024 EUR{OMZET24}; bruto **EUR{BRUTO}** DROP {BRUTO_PCT}% (bruto≫omzet ~{RATIO}x); pnl **EUR{PNL}** DROP {PNL_PCT}% vs YE2024 EUR{PNL24}; equity **EUR{EQUITY}** DROP {EQUITY_PCT}%; FTE **{FTE}**; neerlegging **09.07.2026**. Strong KBO Actief 1 VE. Assets/debt Unknown. Medium. FOI via info@leserables.be.
- Wrote: sources (+5); budgets (+6); commitments (+1); leaderboard (+1 pi 6.15); entities (+1 {ENTITY}); foi + draft {GAP}; rq_2259=done + rq_2260 open (EVERY-10); loop_state ticks={TICK}; raw docs/doge/raw/tick2259/.
...
def main():
        ...
        [
            {
                ...
                "notes": (
                    ...
                    f"bruto {BRUTO} FTE {FTE}; neerlegging 09.07.2026; assets/debt Unknown; "
            ...
            },
            {
                ...
                "title": f"KBO Les Erables/C.A.V.A. {KBO} Actief Tournai 1 VE",
                "url": f"https://kbopub.economie.fgov.be/kbopub/zoeknummerform.html?lang=nl&nummer={KBO_BARE}",
                "publisher": "KBO FOD Economie",
                "accessed_date": DATE,
                "source_class": "official_register",
                "notes": (
                    f"tick{TICK}; Actief VZW/ASBL Les Erables (afkorting C.A.V.A.); "
                    f"zetel Rue du Bois des Hospices 5 7522 Tournai; 1 VE; NACE 88.993/88.995; "
                    f"begindatum 24.06.1991; replaces 0445.138.938; KBO email empty"
                ),
            },
            {
                "source_id": f"src_erables_site_contact_{TICK}",
                "title": "Les Erables FOI channel info@leserables.be",
                "url": "https://www.leserables.be/fr/",
                "publisher": "Les Erables ASBL",
                "accessed_date": DATE,
                "source_class": "foi_contact",
                "notes": (
                    f"tick{TICK}; info@leserables.be / jobs@leserables.be / oh@leserables.be; "
                    "Rue du Bois des Hospices 5 7522 Tournai; also leseta.be annuaire"
                ),
            },
        ],
    )
    ...
    append_csv(
        ...
        [
            ...
            {
                "budget_id": "bud_erables_pnl_jr2024_statutory_cmp",
                "entity_id": ENTITY,
                "year": "2024",
                ...
                "source_id": SRC_EN,
                "confidence": "medium",
                "notes": f"tick{TICK}; YE2024 pnl {PNL24} comparative (pre DROP -89%)",
            },
        ],
    )
        ...
        [
            {
                ...
                "title": (
                    ...
                    f"(omzet 4.63m / pnl DROP {PNL_PCT}% / bruto~{RATIO}x / FTE {FTE} / Medium)"
                ),
                "entity_id": ENTITY,
                ...
                "stated_goal": "Walloon ETA insulation/calorifuge / textile / packaging Tournai",
                "cut_option": (
                    "Publish NBB PDF assets/debt FOI; reconcile pnl DROP -89% vs AVIQ ETA subsidy matrix; "
                ...
                "notes": (
                    ...
                    f"FTE {FTE}; 1 VE; after Val du Geer@2258; AGB Bornem JR2024; FARO/AIESH/REW YE2024; "
        ...
        [
            {
                "item_id": LB,
                "name": (
                    f"Les Erables omzet 4.63m / pnl DROP {PNL_PCT}% / bruto~{RATIO}x / FTE {FTE} "
                    "(YE2025 Walloon ETA)"
                ),
                "level": "L5",
                ...
                "tco_notes": (
                    ...
                    f"equity DROP {EQUITY} / FTE {FTE} / 1 VE Walloon ETA"
                ),
                "confidence": "medium",
                "source_id": SRC_EN,
                "beneficiaries": "ETA workers Tournai / AVIQ adapted-work public path",
                "stated_goal": "Walloon ETA insulation/calorifuge / textile / packaging",
                "measured_outcome": (
                    f"omzet DROP {OMZET_PCT}%; bruto DROP {BRUTO_PCT}%; pnl DROP {PNL_PCT}%; "
                    f"equity DROP {EQUITY_PCT}%; FTE {FTE}; filed 09.07.2026"
                ),
                "absurdity_score": "7.8",
                "cost_score": "4.7",
                "difficulty": "3.0",
                ...
                "cut_proposal": (
                    ...
                    "reconcile pnl DROP -89% + bruto≫omzet"
                ),
                "status": "open",
                "struck_reason": "",
                "notes": (
                    f"tick{TICK} primary; Medium CW; FOI {GAP}; preferred stalls AGB Bornem JR2024; "
                    "FARO/AIESH/REW YE2024; after Val du Geer@2258"
                ),
            }
        ],
    )
    ...
    append_csv(
        ...
        [
            {
                "entity_id": ENTITY,
                "name_nl": "Les Erables / C.A.V.A. VZW (Doornik / ETA maatwerk Henegouwen)",
                ...
                "website": "https://www.leserables.be/fr/",
                ...
                "notes": (
                    ...
                    f"omzet DROP {OMZET} bruto {BRUTO} (~{RATIO}x) pnl DROP {PNL} ({PNL_PCT}%) equity DROP {EQUITY} "
                    f"FTE {FTE}; neerlegging 09.07.2026; assets/debt Unknown; FOI {GAP}; "
                    "preferred stalls AGB Bornem JR2024; FARO/AIESH/REW YE2024; after Val du Geer@2258; "
                    "not TE-additive of 348bn"
                ),
            }
        ],
    ...
    append_csv(
        ...
        [
            {
                "gap_id": GAP,
                "hierarchy_path": "Wallonie>Hainaut>Tournai>LesErables>NBB_PDF_assets_debt_pnl_drop_89pct",
                "entity_id": ENTITY,
                "what_is_missing": (
                ...
                "why_it_matters": (
                    f"Medium CW shows Walloon ETA ASBL (omzet 4.63m / pnl DROP {PNL_PCT}% / FTE {FTE}) "
                ...
                "notes": (
                    ...
                    "AGB Bornem JR2024; after Val du Geer@2258; next EVERY-10 2260"
                ),
            }
        ],
    )
