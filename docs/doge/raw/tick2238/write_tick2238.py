# tick2238 — Manufast-ABP YE2025 Medium leftover dual (FREE Brussels ETA)
...
for sid, title, url, publisher, sclass, notes in [
    ...
    (
        ...
        "official_register",
        (
            "tick2238; Actief ASBL MANUFAST-ABP; zetel Gentsesteenweg/Chaussée de Gand 1434 "
            "1082 Sint-Agatha-Berchem; 1 VE; NACE RSZ/BTW 88.993; Brussels CoCoF ETA"
        ),
    ),
    (
        "src_manufast_site_contact_2238",
        "Manufast FOI channel info@manufast.be",
        "https://www.manufast.be/",
        "Manufast-ABP ASBL",
        "foi_contact",
        "tick2238; info@manufast.be; 02 464 26 11; Chaussée de Gand 1434 1082 Brussels",
    ),
]:
    upsert(
        sources,
...
upsert(
    ...
    ENTITY,
    {
        ...
        "notes": (
            ...
            "Heropbeuring CW opaque; not TE-additive of 348bn"
        ),
    },
)
...
upsert(
    ...
    COMM,
    {
        ...
        "notes": (
            ...
            f"pnl LOSS FLIP; equity DROP -27.53%; FTE DROP {FTE25}; 1 VE; after EntrAnam@"
            "2237; AGB Bornem JR2024; FARO/AIESH/REW YE2024; Heropbeuring CW opaque; "
            "not TE-additive"
        ),
    },
)
...
upsert(
    ...
    LB,
    {
        ...
        "notes": (
            ...
            "AIESH YE2024 / REW YE2024; AGB Bornem JR2024; after EntrAnam@2237; next every-10 2240"
        ),
    },
)
...
upsert(
    ...
    "rq_2238",
    {
        ...
        "hierarchy_target": "L5",
        "entity_id": ENTITY,
        "instructions": "leftover dual after EntrAnam; unused Brussels ETA YE2025",
        "blocked_gap_id": GAP,
        ...
        "notes": (
            ...
            "Heropbeuring CW opaque; next rq_2239; every-10 next 2240"
        ),
    },
)
upsert(
    ...
    "rq_2239",
    {
        "task_id": "rq_2239",
        "title": (
            "leftover dual hole-fill after Manufast — prefer AGB/FARO-YE2025/AIESH-REW/"
        ...
        "entity_id": "",
        "instructions": (
            "leftover dual after Manufast-ABP YE2025 Medium (bruto 6.25m / bruto≫omzet "
            "~1.87x / pnl LOSS FLIP / equity DROP -27%). Prefer leftover AGB/APB if JR2025 "
            "PDF live, else FARO if TRUE NBB YE2025, else AIESH/REW if YE2025, else "
            "Heropbeuring if NBB/CW euros live, else unused maatwerk/kringloop/WZC/IGS/HVZ/"
            "ETA (APN YE2024-only; ETA Beauraing/Perron/Gielen candidates). Do NOT redo "
            "Manufast, EntrAnam, Enghien, Entra, Ateliers de Tertre, Le Rucher, Travie, SDB, "
        ...
        "updated_utc": UTC,
        "notes": (
            "spawned after tick2238 Manufast; FARO/AIESH/REW YE2024; AGB Bornem JR2024; "
...
ls[0] = {
    ...
    "paused": "no",
    "notes": (
        f"tick2238 leftover Manufast 0409.118.977 Medium (bruto {BR25} ~{RATIO}x omzet "
        f"{OM25}; pnl LOSS FLIP {PN25}; equity DROP {EQ25} -27.53%; FTE DROP {FTE25}; "
        "1 VE Brussels ETA); after EntrAnam@2237; AGB Bornem JR2024; FARO/AIESH/REW YE2024; "
        "Heropbeuring CW opaque; next rq_2239; next every-10 2240; continuous hole_fill"
    ),
}
...
## Tick 2238 - 2026-08-27T01:30:00Z - rq_2238 Manufast-ABP Brussels (bruto 6.25m / bruto≫omzet ~1.87x / pnl LOSS FLIP / equity DROP -27% / Medium)

- Unit: **rq_2238** leftover dual after **rq_2237 EntrAnam**. Prefer NON-stall live: AGB Bornem still **JR2024-only**; FARO still **YE2024**; AIESH still **YE2024**; REW still **YE2024**; Heropbeuring still **CW opaque**. Took FREE unused Brussels ETA **Manufast-ABP ASBL** YE2025 (KBO **0409.118.977**; Chaussée de Gand 1434 Berchem-Sainte-Agathe; **Actief** **1 VE**; NACE **88.993**). Do not redo EntrAnam/Enghien/Entra/Ateliers Tertre stack.
...
- Wrote: sources (+5); budgets (+6); commitments (+1); leaderboard (+1 pi 6.30); entities (+1 vzw_manufast_abp_brussel); foi + draft {GAP}; rq_2238=done + rq_2239 open; loop_state ticks=2238; raw docs/doge/raw/tick2238/.
...
print(
    f"OK tick2238 Manufast bruto={BR25} omzet={OM25} ratio={RATIO} pnl={PN25} "
